...
if (description)
{
  ...
  script_cve_id("CVE-2019-18425");
  script_xref(name:"IAVB", value:"2019-B-0084-S");

  script_name(english:"Xen Project Descriptor Table Limit Checking Privilege Escalation Vulnerability (XSA-298)");
...
the remote host is affected by a privilege escalation vulnerability due to
improper bounds checking for descriptor table accesses. An authenticated
attacker, in user mode on a 32-bit guest, could leverage this vulnerability to
elevate privileges to that of the guest kernel. Only 32-bit PV guest user mode
can leverage this vulnerability.
...
applied manually to the source code before a recompile and reinstall.");
  script_set_attribute(attribute:"see_also", value:"https://xenbits.xen.org/xsa/advisory-298.html");
