...
if (description)
{
  ...
  script_cve_id("CVE-2020-25595");
  script_xref(name:"IAVB", value:"2020-B-0056-S");

  script_name(english:"Xen PCI Passthrough Code Reading Back Hardware Registers DoS (XSA-337)");
...
of service (DoS) vulnerability. Code paths in Xen's MSI handling have been identified which act on unsanitized values
read back from device hardware registers.  While devices strictly compliant with PCI specifications shouldn't be able to
affect these registers, experience shows that it's very common for devices to have out-of-spec 'backdoor' operations
which can affect the result of these reads. A not fully trusted guest may be able to crash Xen, leading to a Denial
of Service for the entire system.  Privilege escalation and information leaks cannot be excluded.

Only systems passing through devices with out-of-spec (backdoor) functionality can cause issues.  Experience shows that 
such out-of-spec functionality is common; unless you have reason to believe that your device does not have such
functionality, it's better to assume that it does.

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version 
number.");
  script_set_attribute(attribute:"see_also", value:"https://xenbits.xen.org/xsa/advisory-337.html");
...
"Apply the appropriate patch according to the vendor advisory.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2020-25595");
