...
multiple vulnerabilities :
  ...
  - A cross-frame scripting vulnerability exists due to
    ...
    concealing the origin of a web site.

  - A cross-site request forgery (XSRF) vulnerability exists
    due to improper validation of HTTP requests.
...
relied only on the devices's self-reported version number.");
  ...
  script_set_attribute(attribute:"see_also", value:"https://seclists.org/fulldisclosure/2015/Feb/99");
  script_set_attribute(attribute:"solution", value:
"Upgrade the Loxone Smart Home Miniserver firmware to version 6.3 or
later.

Note that the two information disclosure vulnerabilities still exist in
firmware version 6.3. We are currently unaware of a solution for these
issues.");
