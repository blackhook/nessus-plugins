...
if (description)
{
  ...
  script_name(english:"Linux: Cisco AnyConnect Secure Mobility Client Arbitrary Code Execution (cisco-sa-anyconnect-ipc-KfQO9QhK)");
...
"Upgrade to the relevant fixed version referenced in Cisco bug ID CSCvv30103");
  ...
  script_dependencies("cisco_anyconnect_client_nix_installed.nbin");
  script_require_keys("installed_sw/Cisco AnyConnect Secure Mobility Client", "Host/local_checks_enabled", "Settings/ParanoidReport");
...
var app_info = vcf::get_app_info(app:'Cisco AnyConnect Secure Mobility Client');

var constraints = [{ 'max_version' : '4.10.0', 'fixed_display' : 'See vendor advisory' }];
