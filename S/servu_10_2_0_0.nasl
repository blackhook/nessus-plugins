...
if (description)
{
  ...
  script_xref(name:"SECUNIA", value:"41015");
  script_xref(name:"SECUNIA", value:"41018");

  script_name(english:"Serv-U < 10.2.0.0");
...
than 10.2.0.0 and is, therefore, potentially affected by the following
issues :

  - It is possible to create a directory, when using virtual
    paths and various combinations of permissions, where the
    end-user does not have permission to create the 
    directory.

  - Certain web client invalid URL parameters could cause the
    affected application to crash.");
...
if (ver_compare(ver:version , fix:'10.2', strict:FALSE) == -1)
{
