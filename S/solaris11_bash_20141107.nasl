...
if (description)
{
  ...
  script_name(english:"Oracle Solaris Third-Party Patch Update : bash (cve_2012_3410_stack_based)");
...
security updates :
  ...
  - Stack-based buffer overflow in lib/sh/eaccess.c in GNU
    Bash before 4.2 patch 33 might allow local users to
    bypass intended restricted shell access via a long
    filename in /dev/fd, which is not properly handled when
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://blogs.oracle.com/sunsecurity/cve-2012-3410-stack-based-buffer-overflow-vulnerability-in-bash"
  );
