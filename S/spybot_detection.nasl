...
kinds from your computer.");
 script_set_attribute(attribute:"see_also", value:"https://www.safer-networking.org/");
 ...
 script_require_keys("SMB/Registry/Enumerated", "SMB/name", "SMB/login", "SMB/password", "SMB/transport");
...
app = 'Spybot Search & Destroy';
cpe = 'cpe:/a:safer-networking:spybot_search_and_destroy';
...
{
  item = pregmatch(pattern:'\\[([^\\]]+)\\]', string:line);
  if(!isnull(item[1]))
  {
  ...
  if (section_name != "")
  {
    item = pregmatch(pattern:'ReleaseDate=([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])', string:line);
    if(!isnull(item[1]))
    {
