...
if (description)
{
  ...
  script_name(english:"SuSE 11.1 Security Update : sudo, sudo-debuginfo, sudo-debugsource (SAT Patch Number 6306)");
...
/usr/bin/X11/*, which is probably not intended. The behaviour was
aligned to the one described in the sudoers manpage"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://bugzilla.novell.com/show_bug.cgi?id=739214"
  );
  script_set_attribute(
    attribute:"see_also",
  ...
  script_set_attribute(attribute:"solution", value:"Apply SAT patch number 6306.");
...
flag = 0;
if (rpm_check(release:"SLED11", sp:1, cpu:"i586", reference:"sudo-1.7.6p2-0.2.8.1")) flag++;
if (rpm_check(release:"SLED11", sp:1, cpu:"x86_64", reference:"sudo-1.7.6p2-0.2.8.1")) flag++;
if (rpm_check(release:"SLES11", sp:1, reference:"sudo-1.7.6p2-0.2.8.1")) flag++;
