...
if (description)
{
  script_id(94900);
  script_version("1.5");
  script_cvs_date("Date: 2019/05/28  6:49:20");
...
vulnerabilities; however, this plugin has not tested for them.");
  ...
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?5f9b4e01");
...
url = '/end-user/ftp_redirect.php?r=1&host=test&msg=test&ip=test&s=www.example.com';
...
pattern = "^<script>window.location.href = 'http://www\.example\.com/end-user/ftp_redirect\.php\?h=&msg=test&STYLE=[a-f0-9]+'</script>$";
