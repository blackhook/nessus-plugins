...
if (description)
{
 ...
 script_name(english:"Microsoft Windows 'Replicator' Group User List");
 script_summary(english:"Lists users that are in special groups");

 script_set_attribute(attribute:"synopsis", value:"There is at least one user in the 'Replicator' group.");
 script_set_attribute(attribute:"description", value:
"Using the supplied credentials, it is possible to extract the member
list of the 'Replicator' group. Members of this group can replicate
(or copy) files or directories within a domain.");
...
sid = raw_string (0x01,0x02,0x00,0x00,0x00,0x00,0x00,0x05,0x20,0x00,0x00,0x00,0x28,0x02,0x00,0x00);
...
if (info)
{
  ...
  report = string(
    "\n",
    report, " of the 'Replicator' group :\n",
    "\n",
    data_protection::sanitize_user_enum(users:info)
  );
