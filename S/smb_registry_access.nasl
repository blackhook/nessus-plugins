...
if (logged == 0)
{
 ...
 if ( !isnull(reason) )
 {
   set_kb_item(name:"SMB/registry_not_accessible/reason", value:reason);
   if (login)
     lcx::log_issue(type:lcx::ISSUES_ERROR, proto:lcx::PROTO_SMB,
       msg:reason, port:port, user:login);
 }
 else if (login)
   lcx::log_issue(type:lcx::ISSUES_ERROR, proto:lcx::PROTO_SMB, msg:
     "The registry is not accessible.", port:port, user:login);
}
else
{
 security_note (port);
