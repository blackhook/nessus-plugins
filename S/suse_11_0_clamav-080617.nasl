...
# extracted from openSUSE Security Update clamav-44.
#
...
if (description)
{
  ...
  script_name(english:"openSUSE Security Update : clamav (clamav-44)");
  script_summary(english:"Check for the clamav-44 patch");
...
file that triggers an out-of-bounds read."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://bugzilla.novell.com/show_bug.cgi?id=399302"
  );
  script_set_attribute(
    attribute:"solution", 
...
if ( rpm_check(release:"SUSE11.0", reference:"clamav-0.93.1-0.1") ) flag++;
if ( rpm_check(release:"SUSE11.0", reference:"clamav-db-0.93.1-0.1") ) flag++;
