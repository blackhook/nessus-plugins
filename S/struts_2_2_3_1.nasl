...
if (description)
{
  ...
  script_name(english:"Apache Struts 2.x < 2.2.3.1 RCE (S2-007)");
...
prior to 2.2.3.1. It, therefore, is affected by a possible remote code
execution vulnerability when user-supplied input is evaluated as an
OGNL expressions when there is a conversion error.

Note that Nessus has not tested for these issues but has instead
relied only on the application's self-reported version number.");
  script_set_attribute(attribute:"see_also", value:"https://cwiki.apache.org/confluence/display/WW/S2-007");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Apache Struts version 2.2.3.1 or later");
...
constraints = [
  { "min_version" : "2.0.0", "max_version" : "2.2.3", "fixed_version" : "2.2.3.1" }
];
