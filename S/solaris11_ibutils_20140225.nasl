...
if (description)
{
  ...
  script_name(english:"Oracle Solaris Third-Party Patch Update : ibutils (cve_2013_2561_link_following)");
...
security updates :
  ...
  # https://blogs.oracle.com/sunsecurity/cve-2013-2561-link-following-vulnerability-in-openfabrics-ibutils
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?48a32f27"
  );
  ...
  script_set_attribute(attribute:"cpe", value:"p-cpe:/a:oracle:solaris:ibutils");
...
if (empty_or_null(egrep(string:pkg_list, pattern:"^ibutils$"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "ibutils");
...
if (flag)
{
  error_extra = 'Affected package : ibutils\n' + solaris_get_report2();
  error_extra = ereg_replace(pattern:"version", replace:"OS version", string:error_extra);
  if (report_verbosity > 0) security_warning(port:0, extra:error_extra);
  else security_warning(0);
  exit(0);
}
else audit(AUDIT_PACKAGE_NOT_AFFECTED, "ibutils");
