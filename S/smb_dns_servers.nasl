...
foreach interface (keys(dns_servers))
{
  ...
  foreach type (keys(dns_servers[interface]))
  {
    if (data_protection::is_sanitize_ipaddr_enabled())
      dns_servers_interface_values = data_protection::sanitize_ip_address_multiple(text:dns_servers[interface][type]);
    else
      dns_servers_interface_values = dns_servers[interface][type];

    report += type + ': ' + dns_servers_interface_values + '\n';
