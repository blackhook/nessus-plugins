...
if (description)
{
 ...
 script_set_attribute(attribute:"plugin_modification_date", value:"2022/02/01");
  ...
  script_xref(name:"IAVT", value:"0001-T-0501");
...
if (isnull(display_names) && isnull(display_vers)) exit(1, "Unable to obtain Uninstall/* registry key information.");

apps = make_array();
foreach key (keys(display_names))
{
  matches = pregmatch(string:key, pattern:"^(SMB/Registry/HKLM/SOFTWARE/Microsoft/Windows/CurrentVersion/Uninstall/.+)/DisplayName$");
...
{
  matches = pregmatch(string:key, pattern:"^(SMB/Registry/HKLM/SOFTWARE/Microsoft/Windows/CurrentVersion/Uninstall/)(.+)/DisplayVersion$");
  if (isnull(matches)) continue; # based on the arg passed to get_kb_list() above, the eregmatch() call should never fail
...
{
  matches = pregmatch(string:key, pattern:"^(SMB/Registry/HKLM/SOFTWARE/Microsoft/Windows/CurrentVersion/Uninstall/)(.+)/InstallDate$");
  if (isnull(matches)) continue; # based on the arg passed to get_kb_list() above, the eregmatch() call should never fail
