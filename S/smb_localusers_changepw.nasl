...
if (description)
{
 ...
 script_name(english:"Microsoft Windows - Local Users Information : Can't Change Password");
 script_summary(english:"Lists local users that can not change their passwords.");

 script_set_attribute(attribute:"synopsis", value:
"At least one local user cannot change his or her password.");
 script_set_attribute(attribute:"description", value:
"Using the supplied credentials, Nessus was able to list local users
who can not change their own passwords.");
...
while(login)
{
 ...
 if(acb)
 {
  if(acb & 0x40) # UF_PASSWD_CANT_CHANGE
  {
    ...
    set_kb_item(name:"SMB/LocalUsers/PwCantChange/"+count, value:login);
	}
 }
 count = count + 1;
 login = get_kb_item(string("SMB/LocalUsers/", count));
