...
if (description)
{
  ...
  script_xref(name:"MSKB", value:"2467173");
  script_xref(name:"MSKB", value:"2538242");
  script_xref(name:"MSKB", value:"2538243");
  script_xref(name:"MSKB", value:"2565057");
  script_xref(name:"MSKB", value:"2565063");
  script_xref(name:"MSKB", value:"2467174");
  script_xref(name:"MSKB", value:"2467175");
...
arbitrary code execution.");
  ...
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?e2e88505");
...
# added paranoid check since the plugin is for old Microsoft Visual Studio
# Microsoft support end date is July 14, 2020
...
if (max_index(keys(installs)) || vcpp2010_installed)
{
  ...
  )
  {
    ...
    if (!isnull(vc2010_syswow_version) &&  vc2010_syswow_version =~ "^10\.0\.4")
    {
      vc2010_syswow_prodname = 'Visual C++ 2010 SP1 Redistributable Package 32-bit';
      kbs[vc2010_syswow_prodname] = '2565063';
      fixed_versions[vc2010_syswow_prodname] = '10.0.40219.325';
    }
    else if (!isnull( vc2010_syswow_version))
    {
      vc2010_syswow_prodname = 'Visual C++ 2010 Redistributable Package 32-bit';
      kbs[vc2010_syswow_prodname] = '2467173';
      fixed_versions[vc2010_syswow_prodname] = '10.0.30319.415';
    }

    fversions[vc2010_syswow_prodname] = vc2010_syswow_version;
    probs[vc2010_syswow_prodname] = 1;
    prod_paths[vc2010_syswow_prodname] = path;
    vuln++;
  }
  ...
  if (!isnull(files))
  {
    foreach file (files)
    {
      if (preg(pattern:"Microsoft\.VC80\.MFC", string:file, icase:TRUE))
      {
        prod = '';
      ...
      else if (preg(pattern:"Microsoft\.VC90\.MFC.+_9\.0\.3[0-9]+", string:file, icase:TRUE))
      {
        prod = '';
...
foreach ver (keys(installs))
{
  ...
  else
  {
    ...
    foreach basedir (basedirs)
    {
      ...
      if (preg(pattern:"\System32$", string:basedir, icase:TRUE))
...
{
  match = pregmatch(pattern:"^(.+)\\(mfc[0-9]+\.dll)$", string:mfc, icase:TRUE);
  if (match)
  {
    path = match[1];
    file = match[2];
