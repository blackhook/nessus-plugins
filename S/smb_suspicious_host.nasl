...
antivirus software, and remove any malicious software.");
  ...
  script_set_attribute(attribute:"cvss_score_rationale", value:"Evidence indicates host may be compromised.");
...
function is_suspicious_entry (line)
{
 ...
 {
   if (
     !preg(pattern:"^[\s\t]*127.0.0.1[\s\t]*teredo.ipv6.microsoft.com", string:line) &&
...
var path = hotfix_get_systemroot();
if (!path) exit(1, "Failed to get the system root.");

var share = hotfix_path2share(path:path);
...
var file =  ereg_replace(pattern:"^[A-Za-z]:(.*)", replace:"\1\system32\drivers\etc\hosts", string:path);
var handle = CreateFile(file: file, desired_access: GENERIC_READ, file_attributes: FILE_ATTRIBUTE_NORMAL, share_mode: FILE_SHARE_READ, create_disposition: OPEN_EXISTING);
if (isnull(handle))
{
 NetUseDel();
...
var fsize = GetFileSize(handle:handle);
var data = NULL;
...
var sfiles = NULL;
var lines, line, data_protection_line, ip_addr, host;
...
foreach line (lines)
{
  ...
  {
    if(data_protection::is_sanitize_ipaddr_enabled())
    {
      data_protection_line = parse_hostfile_line(line:line);
      foreach ip_addr (keys(data_protection_line))
      {
        foreach host (keys(data_protection_line[ip_addr]))
        {
          sfiles += data_protection::sanitize_ip_address(text:ip_addr) + ' ' + host + '\n';
        }
      }
    }
    else
...
if (sfiles)
{
  if (report_verbosity > 0)
  {
    ...
    if ((!get_kb_item("global_settings/enable_plugin_debugging") &&
        ...
        data_protection::is_sanitize_ipaddr_enabled()) # Don't attach hosts file if GDPR
    {
