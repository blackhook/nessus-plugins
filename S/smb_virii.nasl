...
# @DEPRECATED@
#
# Disabled on 2019/08/08 due to the false positive rate and age of the plugin.
...
if (description)
{
  script_id(11329);
  script_version("1.84");
  script_cvs_date("Date: 2019/08/21  8:39:57");

  script_name(english:"Virus Infection Detection (deprecated)");
  script_summary(english:"Checks for the presence of different viruses on the remote host");
...
Note: This plugin has been deprecated due to the false positive rate
and age of the plugin.
");
 ...
 script_set_attribute(attribute:"plugin_publication_date", value:"2003/03/08");
  ...
  script_dependencies("netbios_name_get.nasl", "smb_login.nasl","smb_registry_access.nasl","smb_hotfixes.nasl");
...
{
  local_var key_h, sz, i, report, value;
...
name[i]		= "W32/Vote-A";
url[i]		= "http://www.nessus.org/u?842b5f9a";
key[i]		= "SOFTWARE\Microsoft\Windows\CurrentVersion\Run";
item[i]		= "Norton.Thar";
exp[i]		= "zacker.vbs";
...
if ( ! rootfile ) exit(0, "Failed to get the system root.");
...
"See also : https://www.symantec.com/security_response/writeup.jsp?docid=2005-042611-0148-99\n",
"See also : http://securityresponse.symantec.com/avcenter/venc/data/trojan.rona.html\n",
"Solution :  Use latest antivirus signatures to clean the machine.\n",
"Risk factor : High");
  security_hole(port:port, extra:report);
}
