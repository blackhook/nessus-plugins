...
if (description)
{
  ...
  script_name(english:"Security Update for .NET Core (October 2018)");
  script_summary(english:"Checks for Windows Install of .NET Core.");

  script_set_attribute(attribute:"synopsis", value:
"The remote Windows host is affected by a .NET Core information
disclosure vulnerability.");
  script_set_attribute(attribute:"description", value:
"The remote Windows host has an installation of .NET Core
with a version of 1.0.x prior to 1.0.13, 1.1.x prior to 1.1.10
or 2.0.x. Therefore, the host is affected by an information
disclosure vulnerability.");
...
"Update to .NET Core Runtime version 1.0.13 / 1.1.10 / 2.1.0 or later.");
...
foreach install (installs[1])
{
  ...
  if      (version =~ '^2\\.0\\.') fix = '2.1.0';
  else if (version =~ '^1\\.1\\.') fix = '1.1.10';
  else if (version =~ '^1\\.0\\.') fix = '1.0.13';

  if (!empty_or_null(fix) && ver_compare(ver:version, fix:fix, strict:FALSE) < 0)
    report +=
      '\n  Path              : ' + path +
