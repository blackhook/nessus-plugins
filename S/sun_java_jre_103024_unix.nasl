...
if (description)
{
  ...
  script_name(english:"Sun Java JRE Font Parsing Privilege Escalation (103024) (Unix)");
...
installed on the remote host reportedly contains an issue in its font
parsing code that may allow an untrusted applet to gain elevated
privileges and, for example read or write local files or execute local
applications.");
  # http://web.archive.org/web/20080206203802/http://sunsolve.sun.com/search/document.do?assetkey=1-26-103024-1
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?6dca6a3e");
...
foreach install (list_uniq(keys(installs)))
{
  ...
  )
  {
    ...
    info += '\n  Fixed version     : 1.5.0_10 / 1.4.2_15\n';
  }
  else if (ver =~ "^[\d\.]+$")
  {
    dirs = make_list(get_kb_list(install));
