...
if (description)
{
  ...
  script_name(english:"Scientific Linux Security Update : firefox on SL7.x x86_64 (20181025)");
...
Bug Fix(es) :
  ...
  - Previously, passwords saved in the Firefox browser and
    encrypted by a master password were erased when Firefox
    was exited. This update ensures that NSS files used to
    decrypt stored login data are handled correctly. As a
    result, the affected passwords are no longer lost after
    restarting Firefox."
  );
  # https://listserv.fnal.gov/scripts/wa.exe?A2=ind1810&L=scientific-linux-errata&F=&S=&P=12458
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?41b48e20"
  );
  script_set_attribute(
    attribute:"solution",
...
flag = 0;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"firefox-60.3.0-1.el7_5", allowmaj:TRUE)) flag++;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"firefox-debuginfo-60.3.0-1.el7_5", allowmaj:TRUE)) flag++;
