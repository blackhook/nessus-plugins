...
PLAIN) is used."
  ...
  script_set_attribute(attribute:"see_also", value:"https://tools.ietf.org/html/rfc4954");
  script_set_attribute(
    attribute:"solution", 
    value:
...
mechanisms only over an encrypted channel."
  ...
  script_dependencies("smtp_authentication.nasl", "ssl_supported_versions.nasl");
...
is_smtps = get_kb_item('SSL/Transport/' + port);
if (isnull(methods) || !isnull(is_smtps))
...
if (report_verbosity > 0)
{
  report =
    ...
    '\nunencrypted channel on port ' + port + ' :' +
    '\n' +
