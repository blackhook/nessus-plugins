...
if (description)
{
  ...
  script_name(english:"Oracle Solaris Third-Party Patch Update : memcached (multiple_vulnerabilities_in_memcached)");
...
security updates :
  ...
  - The process_bin_delete function in memcached.c in
    ...
    buffer over-read when printing to stderr.
    (CVE-2013-0179)

  - memcached before 1.4.17 allows remote attackers to
    bypass authentication by sending an invalid request with
    SASL credentials, then sending another request with
    incorrect SASL credentials. (CVE-2013-7239)
  ...
  - memcached before 1.4.17, when running in verbose mode,
    ...
    vulnerability than CVE-2013-0179 and CVE-2013-7290.
    (CVE-2013-7291)"
  );
  ...
  # https://blogs.oracle.com/sunsecurity/multiple-vulnerabilities-in-memcached
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?605ce710"
  );
  ...
  script_set_attribute(attribute:"cpe", value:"p-cpe:/a:oracle:solaris:memcached");
...
if (empty_or_null(egrep(string:pkg_list, pattern:"^memcached$"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "memcached");
...
if (flag)
{
  error_extra = 'Affected package : memcached\n' + solaris_get_report2();
  error_extra = ereg_replace(pattern:"version", replace:"OS version", string:error_extra);
  if (report_verbosity > 0) security_warning(port:0, extra:error_extra);
  else security_warning(0);
  exit(0);
}
else audit(AUDIT_PACKAGE_NOT_AFFECTED, "memcached");
