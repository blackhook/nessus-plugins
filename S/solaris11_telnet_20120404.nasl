...
if (description)
{
  ...
  script_name(english:"Oracle Solaris Third-Party Patch Update : telnet (cve_2011_4862_buffer_overflow)");
...
security updates :
  ...
  # https://blogs.oracle.com/sunsecurity/cve-2011-4862-buffer-overflow-vulnerability-in-telnet
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?4e4b6f56"
  );
  ...
  script_set_attribute(attribute:"cpe", value:"p-cpe:/a:oracle:solaris:telnet");
...
if (empty_or_null(egrep(string:pkg_list, pattern:"^telnet$"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "telnet");
...
if (flag)
{
  error_extra = 'Affected package : telnet\n' + solaris_get_report2();
  error_extra = ereg_replace(pattern:"version", replace:"OS version", string:error_extra);
  if (report_verbosity > 0) security_hole(port:0, extra:error_extra);
  else security_hole(0);
  exit(0);
}
else audit(AUDIT_PACKAGE_NOT_AFFECTED, "telnet");
