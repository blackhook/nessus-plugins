...
if (description)
{
 ...
 script_name(english:"Symantec Ghost Solution Status Server Detection");
 script_set_attribute(attribute:"synopsis", value:
"An OS deployment server is running on the remote port." );
 script_set_attribute(attribute:"description", value:
"The remote host is running the 'status' server of Symantec Ghost
solution, an OS deployment and management solution. 

This service is used by clients to ask if a management task has to be
executed." );
...
s[7] = mklist("IPADDRESS", mkipaddr(compat::this_host()));
...
if ("UserInterfaceEnabled" >< buf && "Timeout" >< buf)
