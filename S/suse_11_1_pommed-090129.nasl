...
# extracted from openSUSE Security Update pommed-486.
#
...
if (description)
{
  ...
  script_name(english:"openSUSE Security Update : pommed (pommed-486)");
  script_summary(english:"Check for the pommed-486 patch");
...
behavior and therefore needs a new configuration."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://bugzilla.novell.com/show_bug.cgi?id=469771"
  );
  script_set_attribute(
    attribute:"solution", 
...
if ( rpm_check(release:"SUSE11.1", reference:"gpomme-1.22-1.15.1") ) flag++;
if ( rpm_check(release:"SUSE11.1", reference:"pommed-1.22-1.15.1") ) flag++;
if ( rpm_check(release:"SUSE11.1", reference:"wmpomme-1.22-1.15.1") ) flag++;
