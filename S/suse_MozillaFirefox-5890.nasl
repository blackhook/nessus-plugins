...
if (description)
{
  ...
  script_name(english:"SuSE 10 Security Update : MozillaFirefox (ZYPP Patch Number 5890)");
  ...
  script_set_attribute(
    ...
    value:
"The Mozilla Firefox browser was updated to version 2.0.0.19, fixing
various security issues and stability problems.

The following security issues were fixed :

  - Mozilla security researcher moz_bug_r_a4 reported
    vulnerabilities in the session-restore feature by which
    content could be injected into an incorrect document
    storage location, including storage locations for other
    domains. An attacker could utilize these issues to
    violate the browser's same-origin policy and perform an
    XSS attack while SessionStore data is being restored.
    moz_bug_r_a4 also reported that one variant could be
    used by an attacker to run arbitrary JavaScript with
    chrome privileges. (MFSA 2008-69 / CVE-2008-5513)
  ...
  - Mozilla security researcher moz_bug_r_a4 reported an
    additional variation on the feed preview vulnerabilities
    fixed in Firefox 2.0.0.17. moz_bug_r_a4 demonstrated
    that it was still possible to use the feed preview as a
    vector for JavaScript privilege escalation. An attacker
    could use this issue to run arbitrary JavaScript with
    chrome privileges. Firefox 3 is not affected by this
    issue. Workaround Disable JavaScript until a version
    containing these fixes can be installed. (MFSA 2008-62 /
    CVE-2008-5504)
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://www.mozilla.org/en-US/security/advisories/mfsa2008-62/"
  );
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://www.mozilla.org/en-US/security/advisories/mfsa2008-69/"
  );
  script_set_attribute(
    attribute:"see_also",
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"http://support.novell.com/security/cve/CVE-2008-5504.html"
  );
  script_set_attribute(
    attribute:"see_also",
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"http://support.novell.com/security/cve/CVE-2008-5513.html"
  );
  script_set_attribute(attribute:"solution", value:"Apply ZYPP patch number 5890.");
...
flag = 0;
if (rpm_check(release:"SLED10", sp:2, reference:"MozillaFirefox-2.0.0.19-0.3")) flag++;
if (rpm_check(release:"SLED10", sp:2, reference:"MozillaFirefox-translations-2.0.0.19-0.3")) flag++;
if (rpm_check(release:"SLES10", sp:2, reference:"MozillaFirefox-2.0.0.19-0.3")) flag++;
if (rpm_check(release:"SLES10", sp:2, reference:"MozillaFirefox-translations-2.0.0.19-0.3")) flag++;
