...
if (!isnull(version) && !isnull(path))
{
  ...
  v = pregmatch(pattern:pat, string:version);
  if (!isnull(v))
  {
    if (ver[0] < 7)
    {
  ...
  set_kb_item(name:"SMB/Acrobat/"+version+"/Track", value:track);

  if (!empty_or_null(su1))
    register_install(
      app_name:app,
      vendor : 'Adobe',
      product : 'Acrobat',
  ...
  if (!empty_or_null(su1))
    report_installs(app_name:app, port:port, extra:'  Update  : Security Update 1 \n');
  else
