...
to use excessive resources. Upgrading to SA 3.4.3 as soon as possible
is the recommended fix but details will not be shared publicly.
(CVE-2019-12420)

In Apache SpamAssassin before 3.4.3, nefarious CF files can be
configured to run system commands without any output or errors. With
this, exploits can be injected in a number of scenarios. In addition
to upgrading to SA 3.4.3, we recommend that users should only use
update channels or 3rd party .cf files from trusted places.
(CVE-2018-11805)"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://alas.aws.amazon.com/ALAS-2020-1341.html"
  );
  script_set_attribute(
    attribute:"solution", 
...
flag = 0;
if (rpm_check(release:"ALA", reference:"spamassassin-3.4.3-2.2.amzn1")) flag++;
if (rpm_check(release:"ALA", reference:"spamassassin-debuginfo-3.4.3-2.2.amzn1")) flag++;
