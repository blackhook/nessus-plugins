...
# @DEPRECATED@
#
# replaced by adobe_reader_apsb10-15.nasl on June 30, 2010
#
exit(0);
...
if (description)
{
  ...
  script_name(english:"Adobe Reader 9 <= 9.3.2 (APSA10-01)");
...
"The remote Windows host contains Adobe Reader version 9.3.2 or
earlier.  Such versions are affected by a vulnerability in the
ActionScript Virtual Machine 2.  The 'newfunction' instruction can be
manipulated to jump to user-controlled memory. 
...
In the meantime, deleting or renaming the version of authplay.dll that
ships with Adobe Reader will result in a non-exploitable crash when
opening PDFs containing SWF content.");
...
# This only affects Reader 9.x <= 9.3.2
if (
  ver[0] != 9 ||
  (ver[0] == 9 && ver[1] > 3) ||
  (ver[0] == 9 && ver[1] == 3 && ver[2] > 2)
) exit(0, 'Adobe Reader '+version+' is not affected.');
...
dll = ereg_replace(pattern:'^[A-Za-z]:(.*)', replace:"\1\authplay.dll", string:path);
...
if (vuln)
{
  ...
  {
    dll_path = path + "\authplay.dll";
    report =
      '\nA vulnerable version of Adobe Reader was detected :\n\n'+
      '  Path              : '+path+'\n'+
...
else exit(0, 'Adobe Reader '+version_ui+' is installed, but authplay.dll has been deleted or renamed so the host is not affected.');
