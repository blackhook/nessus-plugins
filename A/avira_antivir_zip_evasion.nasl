...
if (description)
{
 ...
 script_cvs_date("Date: 2018/11/15 20:50:26");
  ...
  script_bugtraq_id(34723);

  script_name(english:"Avira AntiVir Zip Scan Evasion");
...
of the installed product is earlier than 7.9.0.148 / 8.2.0.148. Such
versions reportedly fail to properly inspect specially crafted 'ZIP'
files. An attacker could embed code in a ZIP file in order to
circumvent detection by antivirus software.");
  script_set_attribute(attribute:"see_also", value:"http://forum.avira.com/wbb/index.php?page=Thread&threadID=88472");
  script_set_attribute(attribute:"see_also", value:"https://www.securityfocus.com/archive/1/503013/30/0/threaded" );
  script_set_attribute(attribute:"solution", value:
"Use the Avira update feature to upgrade the scan engine to 7.9.0.148 /
8.2.0.148 or later.");
  ...
  foreach prod (keys(paths))
  {
    ...
    if((!isnull(ver_app))&&(!isnull(ver_engine)))
    {
      ...
      if (
        (
          app_version =~ "^7.*" &&
          ver_engine =~ "^[0-6]\.[0-9\.]+|7\.([0-8]\.[0-9\.]+|9\.0\.([0-9]{1,2}|1([0-3][0-9]|4[0-7]))$)"
         ) ||
         (
           app_version =~ "^[89].*" &&
           ver_engine =~ "^[0-7]\.[0-9\.]+|8\.([01]\.[0-9\.]+|2\.0\.([0-9]{1,2}|1([0-3][0-9]|4[0-7]))$)"
         )
      )
      {
