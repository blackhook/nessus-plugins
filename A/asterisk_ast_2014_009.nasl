...
if (description)
{
  ...
  script_cve_id("CVE-2014-6609");
  script_bugtraq_id(69972);

  script_name(english:"Asterisk PJSIP Channel Driver DoS (AST-2014-009)");
...
service vulnerability in the PJSIP channel driver. This is due to a
flaw in the 'res_pjsip_pubsub' module. The issue is triggered when
handling a SIP SUBSCRIBE request with unexpected mixes of headers for
a given event package. By using a specially crafted request, a remote
attacker could exploit this to crash the application.

Note that Nessus has not tested for this issue but has instead relied
only on the application's self-reported version number.");
  script_set_attribute(attribute:"see_also", value:"http://downloads.asterisk.org/pub/security/AST-2014-009.html");
  script_set_attribute(attribute:"see_also", value:"https://issues.asterisk.org/jira/browse/ASTERISK-24136");
...
"Upgrade to Asterisk 12.5.1 or apply the appropriate patch listed in
the Asterisk advisory.");
  script_set_cvss_base_vector("CVSS2#AV:N/AC:L/Au:S/C:N/I:N/A:P");
  script_set_cvss_temporal_vector("CVSS2#E:U/RL:OF/RC:C");
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2014-6609");
...
foreach kb_name (keys(asterisk_kbs))
{
  ...
  # Open Source 12.1.x < 12.5.1
  fixed = "12.5.1";

  if (
    version =~ "^12\.[1-9][0-9]*($|[^0-9])" &&
