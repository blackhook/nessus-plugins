...
if (description)
{
 ...
 script_cvs_date("Date: 2018/08/07 16:46:51");
  ...
  script_bugtraq_id(35008);

  script_name(english:"Avira AntiVir PDF Scan Evasion");
...
of the installed product is earlier than 7.9.0.168 / 8.2.0.168. Such
versions reportedly fail to properly inspect specially crafted 'PDF'
files. An attacker could embed code in a PDF file in order to
circumvent detection by antivirus software.");
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?6abd005d");
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?c91874c2" );
  script_set_attribute(attribute:"solution", value:
"Use the Avira update feature to upgrade the scan engine to 7.9.0.168 /
8.2.0.168 or later.");
  ...
  foreach prod (keys(paths))
  {
    ...
    if((!isnull(ver_app))&&(!isnull(ver_engine)))
    {
      ...
      if (
        (
          app_version =~ "^7.*" &&
          ver_engine =~ "^[0-6]\.[0-9\.]+|7\.([0-8]\.[0-9\.]+|9\.0\.([0-9]{1,2}|1([0-5][0-9]|6[0-7]))$)"
         ) ||
         (
           app_version =~ "^[89].*" &&
           ver_engine =~ "^[0-7]\.[0-9\.]+|8\.([01]\.[0-9\.]+|2\.0\.([0-9]{1,2}|1([0-5][0-9]|6[0-7]))$)"
         )
      )
      {
