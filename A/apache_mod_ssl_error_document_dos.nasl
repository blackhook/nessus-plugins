...
the entire server.");
  script_set_attribute(attribute:"see_also", value:"https://bz.apache.org/bugzilla/show_bug.cgi?id=37791" );
...
if (ver_compare(ver:version, fix:"2.0.58") == -1)
{
  ...
  for (iter = 0; iter < tries; iter++)
  {
    ...
    if (soc)
    {
      ...
      if (res) audit(AUDIT_LISTEN_NOT_VULN, "Apache", port, install["version"]);
    }
  }
