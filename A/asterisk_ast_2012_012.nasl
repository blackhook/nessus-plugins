...
if (description)
{
  ...
  script_bugtraq_id(55351);

  script_name(english:"Asterisk Manager Interface ExternalIVR Application Originate Action Remote Shell Command Execution (AST-2012-012)");
...
controls, execute shell commands and escalate privileges.

Applications and functions that require 'system' class authorization are
contained in a predefined list, which allows or denies certain actions.
The combination of the 'ExternalIVR' application and the 'Originate'
action is not in the predefined list and can be used to execute
unauthorized shell commands on the host.");
  script_set_attribute(attribute:"see_also", value:"http://downloads.asterisk.org/pub/security/AST-2012-012.html");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Asterisk 1.8.15.1 / 10.7.1, Business Edition C.3.7.6,
Certified Asterisk 1.8.11-cert6 or apply the patches listed in the
Asterisk advisory.");
...
foreach kb_name (keys(asterisk_kbs))
{
  ...
  # Asterisk Certified 1.8.11-certx < 1.8.11-cert6
