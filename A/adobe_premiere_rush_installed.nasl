...
if (description)
{
  ...
  script_name(english:"Adobe Premiere Rush Installed");
...
"Adobe Premiere Rush, a video editing application, is installed on the remote host.");
  script_set_attribute(attribute:"see_also", value:"https://www.adobe.com/products/premiere-rush.html");
...
{
  if (subkey =~ '^Adobe Premiere Rush( CS[0-9]+)?.exe$')
  {
...
foreach exe (exes)
{
  ...
  set_kb_item(name:'SMB/Adobe_Premiere_Rush/'+version+'/Path', value:path);
  set_kb_item(name:'SMB/Adobe_Premiere_Rush/'+version+'/ExePath', value:exe);
...
if (install_count)
{
  ...
  set_kb_item(name:'SMB/Adobe_Premiere_Rush/installed', value:TRUE);
