...
# extracted from F5 Networks BIG-IP Solution K82508682.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Linux kernel vulnerability (K82508682)");
...
kernel through 4.9.11 mishandles DCCP_PKT_REQUEST packet data
structures in the LISTEN state, which allows local users to obtain
root privileges or cause a denial of service (double free) via an
application that makes an IPV6_RECVPKTINFO setsockopt system call.
(CVE-2017-6074)"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K82508682"
  );
  script_set_attribute(
    attribute:"solution",
    value:
...
vmatrix["LC"]["unaffected"] = make_list("13.0.0HF1","12.1.2HF1","11.5.5");

# LTM
vmatrix["LTM"] = make_array();
vmatrix["LTM"]["affected"  ] = make_list("13.0.0","12.0.0-12.1.2","11.4.0-11.6.1","11.2.1");
vmatrix["LTM"]["unaffected"] = make_list("13.0.0HF1","12.1.2HF1","11.5.5");

# PEM
vmatrix["PEM"] = make_array();
vmatrix["PEM"]["affected"  ] = make_list("13.0.0","12.0.0-12.1.2","11.4.0-11.6.1");
vmatrix["PEM"]["unaffected"] = make_list("13.0.0HF1","12.1.2HF1","11.5.5");
