...
# extracted from F5 Networks BIG-IP Solution K51351360.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Websocket profile vulnerability (K51351360)");
  ...
  script_set_attribute(
    ...
    value:
"In F5 BIG-IP 12.1.0 through 12.1.2, specific websocket traffic
patterns may cause a disruption of service for virtual servers
configured to use the websocket profile. (CVE-2016-9253)

Note : Virtual servers configured to use the HTTP profile (and no
websocket profile) that process websocket traffic are not vulnerable
to this issue. This issue only affects the BIG-IP data plane; there is
no control plane exposure to this vulnerability.

Impact

An attacker may be able to disrupt service on the BIG-IP system by
sending crafted websocket traffic to the virtual server."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K51351360"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
vmatrix["AFM"]["unaffected"] = make_list("13.0.0","12.1.2HF1","12.0.0","11.4.0-11.6.1","11.2.1");
...
vmatrix["PEM"]["unaffected"] = make_list("13.0.0","12.1.2HF1","12.0.0","11.4.0-11.6.1","11.2.1");
