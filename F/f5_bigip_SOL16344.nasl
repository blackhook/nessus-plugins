...
# extracted from F5 Networks BIG-IP Solution SOL16344.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Apache Tomcat vulnerability (SOL16344)");
  ...
  script_set_attribute(
    ...
    value:
"ava/org/apache/coyote/http11/filters/ChunkedInputFilter.java in Apache
Tomcat 6.x before 6.0.42, 7.x before 7.0.55, and 8.x before 8.0.9 does
not properly handle attempts to continue reading data after an error
has occurred, which allows remote attackers to conduct HTTP request
smuggling attacks or cause a denial of service (resource consumption)
by streaming data with malformed chunked transfer coding."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K16344"
  );
  script_set_attribute(
    attribute:"solution",
    value:
