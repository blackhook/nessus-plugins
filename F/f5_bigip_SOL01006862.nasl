...
# extracted from F5 Networks BIG-IP Solution K01006862.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : cURL and libcurl vulnerability (K01006862)");
  ...
  script_set_attribute(
    ...
    value:
"A flaw was found in curl before version 7.51. If cookie state is
written into a cookie jar file that is later read back and used for
subsequent requests, a malicious HTTP server can inject new cookies
for arbitrary domains into said cookie jar.(CVE-2016-8615)

Impact

When a cURL connection stores a cookie state and is written into a
cookie jar file that is later used for the subsequent cURL requests, a
malicious web server can inject new cookies into the affected cookie
jar for arbitrary domains. This exploit requires access to a malicious
web server that serves cookies."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K01006862"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
