...
# extracted from F5 Networks BIG-IP Solution K34341852.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Apache Tomcat 6.x vulnerability (K34341852)");
...
7.0.67, 8.x before 8.0.30, and 9.x before 9.0.0.M2 processes redirects
...
that lacks a trailing / (slash) character. (CVE-2015-5345)
...
The Tomcat service is not directly exposed to the network, as it only
listens on a localhost socket with connections brokered through the
Apache front-end web service.
...
service is accessible only from the internal network."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K34341852"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
