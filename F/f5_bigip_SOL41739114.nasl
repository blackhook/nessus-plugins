...
# extracted from F5 Networks BIG-IP Solution K41739114.
#
...
if (description)
{
  ...
  script_bugtraq_id(69799);

  script_name(english:"F5 Networks BIG-IP : Linux kernel vulnerability (K41739114)");
...
way the Linux kernel's Universal Disk Format (UDF) file system
implementation processed indirect Information Control Blocks (ICBs).
An attacker with physical access to the system could use a specially
crafted UDF image to crash the system. (CVE-2014-6410)"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K41739114"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
