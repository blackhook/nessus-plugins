...
# extracted from Fedora Security Advisory 2011-13388.
#
...
if (description)
{
  ...
  script_name(english:"Fedora 15 : NetworkManager-0.9.1.90-1.git20110927.fc15 (2011-13388)");
...
In addition, it updates to 0.9.1.90 featuring :
  ...
  - ability to delete connections from nmcli
    ...
    - correctly handles IPv6 link-local DNS servers when
      using the dnsmasq local caching nameserver plugin
...
without introducing additional issues."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://bugzilla.redhat.com/show_bug.cgi?id=737338"
  );
  # https://lists.fedoraproject.org/pipermail/package-announce/2011-September/066425.html
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?6ba87026"
  );
  script_set_attribute(
    attribute:"solution", 
...
flag = 0;
if (rpm_check(release:"FC15", reference:"NetworkManager-0.9.1.90-1.git20110927.fc15")) flag++;
