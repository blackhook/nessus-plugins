...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 25.0.0.148 Multiple Vulnerabilities (APSB17-15)");
...
is equal or prior to version 25.0.0.148. It is, therefore, affected by
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 25.0.0.148
    if(ver_compare(ver:ver,fix:"25.0.0.148",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if(variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 25.0.0.171";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 25.0.0.171 (Chrome PepperFlash)';
      else if(!isnull(fix))
        info += '\n  Fixed version     : ' + fix;
      info += '\n';
    }
