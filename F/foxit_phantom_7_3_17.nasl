...
if (description)
{
  ...
  script_name(english:"Foxit PhantomPDF < 7.3.17 Multiple Vulnerabilities");
...
7.3.17. It is, therefore, affected by multiple vulnerabilities when
not running in Safe-Reading-Mode:
  - Command Injection through a javascript app.launchURL call which 
executes a local program.
  - Arbitrary File Write exposing the ability to drop a file to the
local file system.
  - Arbitrary Write resulting in an executable file on the local file
system."
);
...
"Upgrade to Foxit PhantomPDF version 7.3.17 or later.");
...
constraints = [
      { 'fixed_version' : '7.3.17', 'max_version': '7.3.15.712' }
];
