...
if (description)
{
  ...
  script_name(english:"Flash Player <= 15.0.0.223 Dereferenced Memory Pointer RCE (APSB14-26)");
...
installed on the remote Windows host is equal or prior to 15.0.0.223.
  ...
  if (!isnull(vers) && !isnull(files))
  {
    foreach key (keys(vers))
    {
      ...
      if (ver)
      {
        ...
        if (
          (
            # Chrome Flash <= 15.0.0.223
            variant == "Chrome_Pepper" &&
            (
              (iver[0] < 15) ||
              (iver[0] == 15 && iver[1] == 0 && iver[2] == 0 && iver[3] <= 223)
            )
          ) ||
          (variant != "Chrome_Pepper" &&
            (
        ...
        )
        {
          ...
          if (variant == "Chrome_Pepper")
            info += '\n  Fixed version     : 15.0.0.239 (Chrome PepperFlash)';
          else
            info += '\n  Fixed version     : '+fix;
          info += '\n';
        }
