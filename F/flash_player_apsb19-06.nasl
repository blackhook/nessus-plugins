...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 32.0.0.114 (APSB19-06)");
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 32.0.0.114
    if (ver_compare(ver:ver,fix:"32.0.0.114",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 32.0.0.142";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 32.0.0.142 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
