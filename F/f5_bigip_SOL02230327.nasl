...
# extracted from F5 Networks BIG-IP Solution K02230327.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : BIND vulnerability (K02230327)");
...
BIND into accepting an unauthorized dynamic update. Affects BIND
9.4.0->9.8.8, 9.9.0->9.9.10-P1, 9.10.0->9.10.5-P1, 9.11.0->9.11.1-P1,
9.9.3-S1->9.9.10-S2, 9.10.5-S1->9.10.5-S2. (CVE-2017-3143)

Impact

BIG-IP

An attacker may be able to manipulate the contents of a zone when the
vulnerability is exploited. For the BIG-IP system to be considered
vulnerable, it must have allowed remote update with TSIG
authentication configured in BIND. This configuration combination is
not a default configuration.

F5 iWorkflow, BIG-IQ, andEnterprise Manager
...
the vulnerability in default, standard, or recommended configurations."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K02230327"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
