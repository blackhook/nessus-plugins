...
# extracted from F5 Networks BIG-IP Solution K84940705.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : cURL and libcurl vulnerability (K84940705)");
  ...
  script_set_attribute(
    ...
    value:
"A flaw was found in curl before version 7.51.0. The way curl handles
cookies permits other threads to trigger a use-after-free leading to
information disclosure. (CVE-2016-8623)

Impact

A use-after-free can occur with shared cookies, allowing a user or
processunauthorized access to view or replace cookies sent by cURL. On
the BIG-IP system, access to the cURL utility is restricted to locally
authenticated users."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K84940705"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
