...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 25.0.0.171 Multiple Vulnerabilities (APSB17-17)");
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 25.0.0.171
    if(ver_compare(ver:ver,fix:"25.0.0.171",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if(variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 26.0.0.126";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 26.0.0.126 (Chrome PepperFlash)';
      else if(!isnull(fix))
        info += '\n  Fixed version     : ' + fix;
      info += '\n';
    }
