...
if (description)
{
  ...
  script_xref(name:"IAVT", value:"0001-T-0602");
...
if (!isnull(chrome_installs))
{
  ...
  foreach chrome_install (keys(chrome_installs))
  {
    ...
    item = pregmatch(string:chrome_install, pattern:"^SMB.*\/([0-9\.]+)([^0-9]|$)");
...
foreach file (keys(variants))
{
  ...
  # CS37669: ARM64 arch targets should check different file
  var arch = get_kb_item("Host/cpu/processor_architecture");
  if (!empty_or_null(arch) && arch == "ARM64")
  {
    spad_log(message:'Special case for Flash on ARM64 architechure');
    if ("System32\\edgehtml.dll" >< file2)
    {
      spad_log(message:'Rather than check file ' + file2);
      file2 = str_replace(string:file2, find:'System32\\edgehtml.dll', replace:'SysWOW64\\Macromed\\Flash\\flash.ocx');
      spad_log(message:' ...check file ' + file2);
    }
  }
...
if (info)
{
  ...
  report = 'Nessus found the following instances of Flash Player installed on the\n' +
    'remote host :\n' +
    '\n' +
    info ;
