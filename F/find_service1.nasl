...
#
if (preg(string: r0, pattern: "^\d\|\d\|\d+\|(?:win32|win64|\*nix|nix)\|\d+\.\d+\.\d+\|.*$"))
{
...
if (substr_at_offset(str:r0, blob:'\r\n\r\nAutonomic Controls Media Control Server version ', offset:0))
{
 ...
 _security_note(port: port, data: 'An Autonomic Controls Media Control Server is running on this port.\nSee http://www.Autonomic-Controls.com/');
}
...
#
if (preg(pattern:'^(Mon|Tue|Wed|Thu|Fri|Sat|Sun|Lun|Mar|Mer|Jeu|Ven|Sam|Dim) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|D[e\xE9]c|F[e\xE9]v|Avr|Mai|Ao[u\xFB]) *(0?[0-9]|[1-3][0-9]) [0-9]+:[0-9]+(:[0-9]+)?( *[ap]m)?( +[A-Z]+)? [1-2][0-9][0-9][0-9].?.?$', string:r0) ||
# No ^ at the begining of the line!
preg(pattern:' ([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] +20[0-3][0-9]-(0?[1-9]|1[0-2])-([0-2]?[0-9]|3[01])[ \t\r\n]*$', string: r0) ||
preg(pattern:' ([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] +20[0-3][0-9]/(0?[1-9]|1[0-2])/([0-2]?[0-9]|3[01])[ \t\r\n]*$', string: r0) ||
#
preg(pattern:'^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] ([0-2]?[0-9]|3[01])\\.(0?[1-9]|1[0-2])\\.20[0-3][0-9][ \t\r\n]*$', string: r0) ||
# Daytime in German
preg(pattern:"^([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] +([0-2][0-9]|3[01])\.(0[1-9]|1[0-2])\.(19|20)?[0-9][0-9][ \t\r\n]*$", string: r0)
|| preg(pattern: '^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] +20[0-3][0-9][.-](0?[1-9]|1[0-2])[.-]([0-2]?[0-9]|3[01])[ \t\r\n]*$', string: r0)
|| preg(pattern: '^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] ([0-2][0-9]|3[01])\\.(0?[1-9]|1[0-2])\\.+20[0-3][0-9][ \t\r\n]*$', string: r0)
|| preg(pattern: '^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9] +20[0-3][0-9]/(0?[1-9]|1[0-2])/([0-2][0-9]|3[01])[ \t\r\n]*$', string: r0)
|| preg(pattern: '^([01]?[0-9]|2[0-3])\\.[0-5][0-9]\\.[0-5][0-9] ([0-2]?[0-9]|3[01])/(0?[1-9]|1[0-2])/+20[0-3][0-9][ \t\r\n]*$', string: r0)
...
if (pgrep(pattern:"^OK WorkgroupShare .+ server ready", string:r0))
{
...
if (preg(string:r0, pattern:"^\-err this server is currently", icase:TRUE))
{
...
)
{
  ...
  _security_note(port:port, data:"The remote service is a ManageSieve server.");
  exit(0);
}
...
)
{
  ...
  _security_note(port:port, data:"The remote service is a Unicenter Remote Control host agent.");
  exit(0);
}
...
)
{
  ...
  _security_note(port:port, data:"Pharos Notify appears to be listening on this port.");
  exit(0);
}
...
if ('\n\nAF\'s backup server ready.\n' >< r0 &&
    pgrep(string: r0, pattern: '^afbackup [1-9][0-9.]+'))
{
...
if (
  stridx(r0, 'PRLT') == 0 &&
  preg(pattern:"^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]", string:substr(r0, 8))
)
{
  var v;

  v = pregmatch(pattern:"^([0-9]+)\.([0-9]+)\.([0-9]+)\.[0-9]", string:substr(r0, 8));
  if (!isnull(v))
  {
...
if (
  ...
  preg(pattern:"^OS: (Linux|windows\.NET) V: [0-9][.0-9]+", string:r0)
)
{
...
if (
  'Deliantra Server' >< r0 &&
  preg(pattern:"#version [0-9]+ [0-9]+ Deliantra Server", string:r0)
)
{
...
if (
  (
    "Righteous Backup" >< r0 &&
    # Nul characters at the beginning of the string break preg() => substr
    preg(pattern:"\(Righteous Backup Linux Agent\) [0-9]+\.[0-9.]+ build [0-9]+", string: substr(r0, 8))
  ) ||
  (
    "Rejecting backup server" >< r0 &&
    preg(pattern:"Rejecting backup server \([0-9.]+\) since it is not in our allow list", string: r0)
  )
)
{
...
if (
  ...
  strlen(r0) >= 48 &&
  pgrep(pattern:"^[0-9a-f]+$", string:substr(r0, 8))
)
{
...
if (
  '\r\nETHMAC ' >< r0 && '\r\nWIFIMAC ' >< r0 &&
  pgrep(pattern:'^ETHMAC [0-9a-fA-F]{2}(:[0-9a-fA-F]{2}){5}', string:r0) &&
  pgrep(pattern:'^WIFIMAC [0-9a-fA-F]{2}(:[0-9a-fA-F]{2}){5}', string:r0)
)
{
...
if (
  ...
  preg(pattern:"^protocol\x08[0-9]+\.[0-9]", string:r0)
)
{
...
if (
  'Barracuda' >< r0 &&
  pregmatch(pattern:"^BCP-[0-9][.0-9]+-Barracuda", string:r0)
)
{
...
if (
  ...
  preg(pattern:"^HMC [0-9][0-9.]+$", string:substr(r0, 2))
)
{
...
if (preg(pattern:"^CH_STATUS [0-9]{1,4}( [0-9]{1,4})? (LOCAL|REMOTE|RECORDING)", string:r0))
{
...
# Submitted by Chris
if (
  preg(pattern:"You have [0-9]+ seconds? to identify\.", string:r0) &&
...
# 
# Submitted by William Kyrouz
if (substr_at_offset(str:r0, blob:mk_unicode_or_null('Dataflight FYI\n\r'), offset:0))
{
...
)
{
  ...
  _security_note(port: port, data:"A Transbase server is running on this port.");
  exit(0);
}
...
# HP (MicroFocus) Perfd Server
# Note: typically associated with port 5227
#
if("Welcome to the perfd server. Hit <RETURN> to continue" >< r)
{
  report_service(port:port, svc:'perfd');
  exit(0);
}
...
# Submitted by Ky6uk.
if (stridx(r, "You are not authorized (" + compat::this_host() + ")\r\n<br>\r\n<a href=") == 0)
{
 register_service(port:port, proto:"http_proxy");
...
if (preg(string: r, pattern: '^SSH-', icase: 0))
{
...
#
# More info: <http://www.eggheads.org/>.
if (
...
# See <http://www.trimble.com/Infrastructure/Trimble-NTRIP-Caster-App.aspx>
...
# CSYNC2
#
# https://github.com/rvanlaar/csync2/blob/master/csync2.c
#
...
if (
  ...
  preg(pattern:"PROTOCOL [0-9]+ [0-9]+ ((AGENT|INSTALL|CRYPT|COMP),)+", string:r)
)
{
...
if (r_len >= 4 && substr(r, 0, 3) == '\xAC\xED\x00\x05' )
{
 register_service(port:port, proto:'java-listener');
  _security_note(port:port, data:'A Java Object Serialization Stream Protocol is listening on this port.');
 exit(0);
}
...
# NetApp Network Data Management Protocol (NDMP)
# Type : get_http
# 0x00:  80 00 00 38 00 00 00 01 5C B9 F3 48 00 00 00 00    ...8....\..H....
...
if (
  (substr(r, 0, 7) == '\x80\x00\x00\x24\x00\x00\x00\x01' && substr(r, 12, 39) == '\x00\x00\x00\x00\x00\x00\x05\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\x00') ||
  (substr(r, 0, 7) == '\x80\x00\x00\x3c\x00\x00\x00\x01' && substr(r, 12, 63) == '\x00\x00\x00\x00\x00\x00\x05\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\x15Connection successful\x00\x00\x00') ||
  (substr(r, 0, 7) == '\x80\x00\x00\x38\x00\x00\x00\x01' && substr(r, 12, 59) == '\x00\x00\x00\x00\x00\x00\x05\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x04\x00\x00\x00\x12Connection refused\x00\x00')
)
{
  register_service(port:port, proto:'netapp_ndmp');
  _security_note(port:port, data:'A NetApp Network Data Management Protocol (NDMP) server is listening on this port.');
  exit(0);
}

exit(0, "The unknown service on port " + port + " was not identified.");
