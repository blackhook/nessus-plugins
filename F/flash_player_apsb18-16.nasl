...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 29.0.0.140 (APSB18-16)");
...
host is equal or prior to version 29.0.0.140. It is therefore 
affected by multiple vulnerabilities.");
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 29.0.0.140
    if (ver_compare(ver:ver,fix:"29.0.0.140",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 29.0.0.171";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 29.0.0.171 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
