...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 31.0.0.153 (APSB18-42)");
...
host is equal or prior to version 31.0.0.153. It is therefore
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 31.0.0.153
    if (ver_compare(ver:ver,fix:"31.0.0.153",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 32.0.0.101";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 32.0.0.101 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
