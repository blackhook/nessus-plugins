...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 32.0.0.238 (APSB19-46)");
...
host is equal or prior to version 32.0.0.238. It is therefore
affected by multiple arbitrary code execution vulnerabilities.");
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 32.0.0.238
    if (ver_compare(ver:ver,fix:"32.0.0.238",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 32.0.0.255";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 32.0.0.255 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
