...
# extracted from F5 Networks BIG-IP Solution K13421245.
#
...
if (description)
{
  ...
  script_cve_id("CVE-2017-6162");

  script_name(english:"F5 Networks BIG-IP : TMM vulnerability (K13421245)");
...
GTM, Link Controller, PEM, Websafe software version 12.0.0 to 12.1.2,
11.6.0 to 11.6.1, 11.4.0 to 11.5.4, 11.2.1, in some cases TMM may
crash when processing TCP traffic. This vulnerability affects TMM via
a virtual server configured with TCP profile. Traffic processing is
disrupted while Traffic Management Microkernel (TMM) restarts. If the
affected BIG-IP system is configured to be part of a device group, it
will trigger a failover to the peer device. (CVE-2017-6162)

Impact

This vulnerability affects the Traffic Management Microkernel (TMM)
througha virtual server configured with a TCP profile. Traffic
processing is disrupted while TMMrestarts. If the affected BIG-IP
system is configured as part of a device group, the systemwill trigger
a failover to the peer device."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K13421245"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
vmatrix["ASM"]["affected"  ] = make_list("12.0.0-12.1.2","11.6.0-11.6.1","11.4.1-11.5.4","11.2.1");
