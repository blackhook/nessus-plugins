...
# extracted from F5 Networks BIG-IP Solution K46123931.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : cURL and libcurl vulnerability (K46123931)");
  ...
  script_set_attribute(
    ...
    value:
"The function `read_data()` in security.c in curl before version 7.51.0
is vulnerable to memory double free.(CVE-2016-8619)

Impact

An attacker may use this vulnerability to exploit the usage of the
cURL command with Kerberos authentication on custom BIG-IP monitors
and/or the command line."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K46123931"
  );
  script_set_attribute(
    attribute:"solution",
    value:
