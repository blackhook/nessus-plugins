...
NNTP, ...)" );
 ...
 script_dependencies("find_service1.nasl"); # cifs445.nasl 
...
var port = get_kb_item("Services/three_digits");
...
# renaming var 'retry' to 'ret' (retry used as global var below) 
if (thorough_tests) var ret = 3;
else ret = 1;
...
var spontaneous = read_answer(socket: soc);
var banner = spontaneous;
...
var help = read_answer(socket: soc);
if (help)
{
...
if (help !~ '^50[0-9]')
{
 ...
 if (
  ...
  (
   ...
   strcat("portmapper tcp PORTMAPPER ", port) >< banner
  )
 )
 {
  register_service(port:port, proto:"imqbrokerd");
  data = "A Message Queue broker is listening on this port.";
  security_report_v4(port: port, severity: SECURITY_NOTE , extra: data);
 }
 # Spontaneous banner:
 ...
 if (banner && stridx(banner, '000 Chat Server v') == 0)
 {
   ...
   data = "SCI Photo Chat Server is listening on this port.";
   security_report_v4(port: port, severity: SECURITY_NOTE , extra: data);
 }
 ...
 if (egrep(pattern:"^200.*Citadel(/UX| server ready)", string:banner) )
 {
  ...
  data = "A Citadel server is running on this port";
  security_report_v4(port: port, severity: SECURITY_NOTE , extra: data);
 }
 ...
 if (egrep(pattern:"^200.*vmajor=[0-9]+;vminor=[0-9]+;cookie=[0-9]+", string:banner) )
 {
  ...
  data = "The remote service is a TPTEST server, used for measuring Internet bandwidth.";
  security_report_v4(port: port, severity: SECURITY_NOTE , extra: data);
 }
}

send(socket: soc, data: 'HELO mail.nessus.org\r\n');
var helo = read_answer(socket: soc);
...
var lhlo = read_answer(socket: soc);
...
var date = read_answer(socket: soc);
...
{
  var req = strcat("AUTHENTICATE ", SCRIPT_NAME, "\r\n");
  send(socket: soc, data:req);
  var res = read_answer(socket: soc);
  if (!isnull(res) && stridx(res, '551 Invalid hexadecimal encoding.  Maybe you tried') == 0)
  {
    register_service(port:port, proto:"tor_cp");
    data = "The remote service appears to be a Tor control port that allows\n" +
...
{
 banner = read_answer(socket: soc);
 var port2 = ftp_pasv(socket: soc);
 if (port2)
 {
  var soc2 = open_sock_tcp(port2);
  if (soc2)
  {
