...
# extracted from F5 Networks BIG-IP Solution K52828640.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : libcurl vulnerability (K52828640)");
  ...
  script_set_attribute(
    ...
    value:
"A flaw was found in curl before version 7.51.0 When re-using a
connection, curl was doing case insensitive comparisons of user name
and password with the existing connections. This means that if an
unused connection with proper credentials exists for a protocol that
has connection-scoped credentials, an attacker can cause that
connection to be reused if s/he knows the case-insensitive version of
the correct password. (CVE-2016-8616)

Impact

An attacker can cause an unused connection with credentials to be
reused if the attacker knows the case-insensitive version of the
correct password. Local access to an affected F5 system is necessary
to trigger the exploit from the affected F5 system."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K52828640"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
