...
attribute. (CVE-2015-8000)

Impact

An attack may cause a denial-of-service (DoS) on the vulnerable BIND
system by requesting a record that contains a malformed class
attribute.

Although the BIG-IP, BIG-IQ, and Enterprise Manager software contains
the vulnerable code, the BIG-IP, BIG-IQ, and Enterprise Manager
systems do not use the vulnerable code in a way that exposes the
vulnerability in the default configuration.

The BIG-IP system must meet both of the following conditions to be
considered vulnerable :

A listener object is configured to use the local BIND. For example :
...
Alternate and Fallback load balancing methods set to None and all
