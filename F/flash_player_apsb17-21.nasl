...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 26.0.0.131 Multiple Vulnerabilities (APSB17-21)");
...
is equal or prior to version 26.0.0.131. It is, therefore, affected by
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 26.0.0.131
    if (ver_compare(ver:ver,fix:"26.0.0.131",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 26.0.0.137";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 26.0.0.137 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
