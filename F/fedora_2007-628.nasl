...
# extracted from Fedora Security Advisory 2007-628.
#
...
if (description)
{
  ...
  script_name(english:"Fedora Core 6 : wireshark-0.99.6-1.fc6 (2007-628)");
...
"Upgrade to 0.99.6 due to multiple security issues.
  ...
  - Wireshark could crash when dissecting an HTTP chunked
    ...
    - Wireshark could crash while reading iSeries capture
      files

    - Wireshark could exhaust system memory while reading a
      malformed DCP ETSI packet

  - Wireshark could loop excessively while reading a
    malformed SSL packet
...
without introducing additional issues."
  ...
  # https://lists.fedoraproject.org/pipermail/package-announce/2007-July/002707.html
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.nessus.org/u?0a8cd516"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
flag = 0;
if (rpm_check(release:"FC6", reference:"wireshark-0.99.6-1.fc6")) flag++;
if (rpm_check(release:"FC6", reference:"wireshark-debuginfo-0.99.6-1.fc6")) flag++;
if (rpm_check(release:"FC6", reference:"wireshark-gnome-0.99.6-1.fc6")) flag++;
