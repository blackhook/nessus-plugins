...
if (description)
{
  ...
  script_set_attribute(
    ...
    value:
"F5 BIG-IP 12.0.0 and 11.5.0 - 11.6.1 REST requests which timeout
during user account authentication may log sensitive attributes such
as passwords in plaintext to /var/log/restjavad.0.log. It may allow
local users to obtain sensitive information by reading these files.
(CVE-2016-6249)

Impact

An authenticated user may be able to gain access to privileged
information by way ofthe Advanced Shell ( bash ) or qkview data."
  );
  script_set_attribute(
    attribute:"see_also",
