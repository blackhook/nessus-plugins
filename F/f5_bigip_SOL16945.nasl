...
# extracted from F5 Networks BIG-IP Solution K16945.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Mailx vulnerabilities (K16945)");
  ...
  script_set_attribute(
    ...
    value:
"CVE-2014-7844 The expand function in fio.c in Heirloom mailx 12.5 and
earlier and BSD mailx 8.1.2 and earlier allows remote attackers to
execute arbitrary commands via shell meta characters in an email
address.

CVE-2004-2771 A flaw was found in the way mailx handled the parsing of
email addresses. A syntactically valid email address could allow a
local attacker to cause mailx to execute arbitrary shell commands
through shell meta characters (CVE-2004-2771) and the direct command
execution functionality (CVE-2014-7844).

Impact

A local attacker can cause mailx to execute arbitrary shell commands
through shell meta-characters. These attacks require a trusted user
with shell access to the system in question."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K16945"
  );
  script_set_attribute(
    attribute:"solution",
    value:
