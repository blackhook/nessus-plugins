...
# extracted from F5 Networks BIG-IP Solution K17517.
#
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : NTP vulnerability (K17517)");
  ...
  script_set_attribute(
    ...
    value:
"Memory leak in the CRYPTO_ASSOC function in ntpd in NTP 4.2.x before
4.2.8p4, and 4.3.x before 4.3.77 allows remote attackers to cause a
denial of service (memory consumption). (CVE-2015-7701)

Impact

An attacker could send packets to ntpd that may, after several days of
ongoing attack, cause it to run out of memory. There is no control
plane exposure in the BIG-IP system when you use a default
configuration, and this issue is exposed only when NTP is configured
to use the Autokey security protocol."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://support.f5.com/csp/article/K17517"
  );
  script_set_attribute(
    attribute:"see_also",
