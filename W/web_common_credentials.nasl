...
"Reconfigure the affected web pages to use a stronger password.");
 ...
 script_set_attribute(attribute:"cvss_score_rationale", value:"Common credentials");
 ...
 script_copyright("This script is Copyright (C) 2010-2021 Tenable Network Security, Inc.");
...
foreach u (sort(protpl))
{
  ...
  spad_log(message:'Sent request: ' + http_last_sent_request());
  spad_log(message:'Received response: ' + obj_rep(w));
  if (isnull(w))
  {
  ...
  foreach i (auth_l)
  {
    ...
    if (w[0] =~ "^HTTP/1\.[01] 200 ")
    {
      ...
      authURL[u] = i;
      spad_log(message:'Found common creds with username: ' + obj_rep(user[i]) + ' and password: ' + obj_rep(pass[1]));
