...
{
  ##
  #  First check the login form
  ##
  var url = "/console/login/LoginForm.jsp";
  var res = http_send_recv3(method: "GET", port:port, item:url, follow_redirect: 1, exit_on_fail:TRUE);
  if (empty_or_null(res))
  {
    spad_log(message:'No response from ' + url + ' using port ' + port + '\n');
  }
  ...
  ##
  #  Second, check alternate path which may provide identification (but not version, sadly)
  ##
  url = "/wls-wsat/CoordinatorPortType";
  res = http_send_recv3(method: "GET", port:port, item:url, follow_redirect: 1, exit_on_fail:TRUE);
  if (empty_or_null(res))
  {
    spad_log(message:'No response from ' + url + ' using port ' + port + '\n');
    ##
    #  Neither page checked reported weblogic.
    #  Maintain existing output with the following
    ##
  ...
  if (res[0] =~ '^HTTP/[0-9.]+ +200' &&
     (res[2] =~ 'weblogic' ||
     (res[2] =~ 'Implementation class' && res[2] =~ 'CoordinatorPortTypePortImpl') ) )
  {
    info = '\nURL     : ' + build_url(port:port, qs:url) +
           '\nVersion : unknown' +
           '\n';
