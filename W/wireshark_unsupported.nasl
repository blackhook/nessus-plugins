...
if (description)
{
  ...
  script_xref(name:"IAVA", value:"0001-A-0624");
...
"Upgrade to a version of Wireshark that is currently supported.");
  ...
  script_set_attribute(attribute:"cvss_score_rationale", value:"Score given for software which will no longer receive security updates.");
...
eos_dates = make_array(
  ...
  "^3\.[01]($|\.)",    'August 28, 2020',
  #2.x
  "^2\.[56]($|\.)",    'October 18, 2020',
  "^2\.[34]($|\.)",    'July 19, 2019',
  "^2\.[12]($|\.)",    'September 7, 2018',
  "^2\.0($|\.)",       'November 18, 2017',
  "^1\.99($|\.)",      'November 18, 2017',
...
withdrawl_announcements = make_array(
  ...
  "^1\.99($|\.)",      'https://wiki.wireshark.org/Development/LifeCycle',
...
supported_versions = '3.2.x, 3.4.x';
...
foreach app (apps)
{
  ...
  # determine port if windows OS based on wireshark detections
  if (!empty_or_null(install["extra_no_report"])
    && !empty_or_null(install["extra_no_report"]["os"])
    && 'win' == install["extra_no_report"]["os"])
  {
