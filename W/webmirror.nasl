...
'Options' section of the client." );
 ...
 script_dependencies("find_service1.nasl", "httpver.nasl", "http_login.nasl", "DDI_Directory_Scanner.nasl", "embedded_web_server_detect.nasl", "waf_detection.nbin", "broken_web_server.nasl");
...
function try_automatic_login(cgi, args, method, passw)
{
  ...
  if (e == "" || e == "OK")
  {
    ...
    cookies = rq["Cookie"];
    if (cookies)
      set_kb_item(name: strcat("/tmp/http/auth/", port), value: 'Cookie: '+cookies);
    else
...
function add_30x(url)
{
 if(isnull(URLs_30x_hash[url]))
 {
  ...
  set_kb_item(name:strcat("www/", port, "/content/30x"), value:url);
  URLs_30x_hash[url]++;
 }
}
...
function add_auth(url, auth)
{
 ...
 if (URLs_auth_hash[url]) return;
  ...
  set_kb_item(name:strcat("www/", port, "/content/auth_required"), value:url);
...
function add_url(url, depth, referer)
{
 ...
 if(isnull(URLs_hash[url]))
 {
  ...
  {
   set_kb_item(name:strcat("www/", port, "/content/extensions/", ext), value:url);
  }
  ...
  if(dir && !Dirs[dir])
  {
   ...
   if ( dir !~ "^/manual" ) # Apache
    set_kb_item(name:strcat("www/", port, "/content/directories"), value:dir);
   if(isnull(URLs_hash[dir]))
   {
    URLs = make_list(URLs, dir);
    if(Apache)URLs  = make_list(URLs,  strcat(dir, "/?D=A"));
    else if(iPlanet)URLs = make_list(URLs,  strcat(dir, "/?PageServices"));
    URLs_hash[dir] =  0;
   }
  }
 }
...
function remove_cgi_arguments(url, current)
{
 ...
 else
 {
  ...
  foreach arg (make_list(cgi_args)) 
  {
   ...
   if (! isnull(v))
   {
     args = strcat(args, v[1] , " [", v[2], "] ");
     add_arg_to_cgi(cgi:cgi, param: v[1], val: v[2]);
   }
   else
   {
     args = strcat(args, arg, " [] ");
...
function _canonical_url(url, current)
{
 ...
 else
 {
  ...
  url = strcat(basename(name:current, level:num_dots), url);
 }

 if(url[0] != "/")
 	return remove_cgi_arguments(url:strcat("/", url), current: current);
 else
 	return remove_cgi_arguments(url:url, current: current);
 }
 return NULL;
...
function parse_java(elements) 
{
    ...
    {
         if (archive)
            set_kb_item(name:strcat("www/", port, "/java_classfile"), value:strcat(codebase,"/",archive));
         if (code)
             set_kb_item(name:strcat("www/", port, "/java_classfile"), value:strcat(codebase,"/",code));
    } 
    else 
    {
         if (archive)
            set_kb_item(name:strcat("www/", port, "/java_classfile"), value:archive);
         if (code)
            set_kb_item(name:strcat("www/", port, "/java_classfile"), value:code);
    }
}
...
function parse_javascript_src(elements, current, depth)
{
  ...
  { 
   set_kb_item(name:strcat("www/", port, "/infected/pages"), value: v);
  }
  ...
  if(Misc[src_page]) return;
    ...
    if ("Index of /" >< data)
    {
	    if("?D=A" >!< src_page && "?PageServices" >!< src_page)
	    	{
             	 misc_report = misc_report + strcat("Directory index found at ", src_page, "\n");
	   	 Misc[src_page] = 1;
    ...
    {
            misc_report = misc_report + strcat("Extraneous phpinfo() script found at ", src_page, "\n"); 
	    Misc[src_page] = 1;
    }
    ...
    if (php_path != data2)
    {
            misc_report = misc_report + strcat("PHP script discloses physical path at ", src_page, " (", php_path, ")\n");
	    Misc[src_page] = 1;
     }
    }
   }
    ...
    {
      misc_report += strcat(src_page, " seems to have been 'encrypted' with HTML Guardian\n");
      guardian ++;
    }
    
    if("CREATED WITH THE APPLET PASSWORD WIZARD WWW.COFFEECUP.COM" >< data)
    {
      misc_report += strcat(src_page, " seems to contain links 'protected' by CoffeCup\n");
      coffeecup++;
    }
    ...
    if (fp_save != data)
     {
            misc_report = misc_report + strcat("FrontPage form stores results in web root at ", src_page, " (", fp_save, ")\n");
	    Misc[src_page] = 1;
     }
   }
}
 ...
 for ( t = 0 ; t < max ; t ++ )
 {
   ...
   if( elements["nasl_token_type"] == "input" ||
       elements["nasl_token_type"] == "textarea" )
    {
      if(elements["name"])
      {
    	 argz += strcat( elements["name"], " [", elements["value"], "] ");
	 ...
	 if ( isnull(pass_l) ) 
	   ...
	   pass_l = make_list(pass_l, elements["name"]);
    	 argz2 += strcat( "Input name : ", elements["name"], "\n");
	 if ( elements["value"] )
	   argz2 += strcat("Default value :  ", elements["value"], "\n");
   ...
   if ( elements["nasl_token_type"] == "/select" )
	{
	 ...
	 argz += strcat(current_select_name, " [", current_select[i], "] ");
...
dirs = get_kb_list(strcat("www/", port, "/content/directories"));
if(dirs) URLs = make_list(URLs, dirs);

dirs = get_kb_list(strcat("www/", port, "/content/directories/require_auth"));
if(dirs) URLs = make_list(URLs, dirs);
...
if(cnt == 1)
{
 ...
 {
  set_kb_item(name:strcat("www/", port, "/password_protected"), value:TRUE);
 }
}
...
foreach foo (keys(CGIs))
{
 ...
 if(!args) args = "";
 set_kb_item(name:strcat("www/", port, "/cgis"), value:strcat(foo, " - ", args));
 if ( strlen(args) > 72 ) args = substr(args, 0, 69) + "...";
  
 if(!report) 
 	report = strcat("The following CGI have been discovered :\n\nSyntax : cginame (arguments [default value])\n\n", foo, " (", args, ")\n");
 else
 	report = strcat(report, foo, " (", args, ")\n");
