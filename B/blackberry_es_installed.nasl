...
messaging and application servers.");
  script_set_attribute(attribute:"see_also", value:"https://www.blackberry.com/us/en/products/endpoint-management/blackberry-uem");
...
if (!isnull(path) && !isnull(subkey))
{
  ...
  key = "SMB/Registry/HKLM/SOFTWARE/Microsoft/Windows/CurrentVersion/Uninstall/" +  subkey +  "/DisplayName";
  ...
  {
    key = "SMB/Registry/HKLM/SOFTWARE/Microsoft/Windows/CurrentVersion/Uninstall/" +  subkey +  "/DisplayVersion";
    ver = get_kb_item(key);
  }
}
...
if (!isnull(key_h))
{
  ...
  if (empty_or_null(ver))
  {
    ...
    if (!empty_or_null(item))
    {
      match = pregmatch(string:item, pattern:"^(12\.[0-9.]+)(?:[^0-9|$])");
...
if (!empty_or_null(name) && !empty_or_null(ver) && !empty_or_null(path))
{
  ...
  register_install(
    vendor:"BlackBerry",
    product:"BlackBerry Enterprise Service",
    app_name:app,
    path:path,
    version:ver,
