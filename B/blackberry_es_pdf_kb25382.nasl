...
if (description)
{
  ...
  script_cve_id("CVE-2010-2604");
  script_bugtraq_id(45753);
  script_xref(name:"IAVA", value:"2011-A-0003");
  script_xref(name:"Secunia", value:"42882");

  script_name(english:"BlackBerry Enterprise Server / Attachment Service PDF Distiller Buffer Overflow (KB25382)");
...
host is reportedly affected by a buffer overflow vulnerability in the
PDF distiller component of the BlackBerry Attachment Service. By
sending a specially crafted PDF file and having it opened on a
BlackBerry smartphone, an attacker may be able to execute arbitrary
code on the system that runs the BlackBerry Attachment Service.");

  script_set_attribute(attribute:"see_also", value:"https://salesforce.services.blackberry.com/kbredirect/viewContent.do?externalId=KB25382");
...
if (isnull(get_kb_item("BlackBerry_ES/AttachmentServer"))) exit(0, "The host isn't affected because BlackBerry Attachment Server isn't installed.");
...
if ("Enterprise Server" >< prod && version =~ '4\\.1')
{
  ...
  if (version =~ '^4\\.1\\.6\\.') fix = '4.1.6.21';
  else if (version =~ '^4\\.1\\.7\\.') fix = '4.1.7.13';
}
else if ("Enterprise Server" >< prod && version =~ '5\\.0')
{
  BES5 = 1;
  if (version =~ '^5\\.0\\.0\\.') fix = '5.0.0.61';
  else if (version =~ '^5\\.0\\.1\\.') fix = '5.0.1.39';
  else if (version =~ '^5\\.0\\.2\\.') fix = '5.0.2.21';
}
