...
if (description)
{
  ...
  script_cve_id("CVE-2009-4778");
  script_bugtraq_id(37167);
  script_xref(name:"Secunia", value:"37562");

  script_name(english:"BlackBerry Enterprise Server / Attachment Service PDF Distiller Unspecified Vulnerabilities (KB19860)");
...
system that runs the BlackBerry Attachment Service.");
  script_set_attribute(attribute:"see_also", value:"https://salesforce.services.blackberry.com/kbredirect/viewContent.do?externalID=KB19860");
...
if (("Enterprise Server" >< prod && !ereg(pattern:"4\.1\.[3-7]|5\.0", string:version)) ||
     ("Professional Software" >< prod && (version =~ " 4\.1\.4"))
   ) exit(1, "Did not detect an affected version.");
...
else if ("Professional Server" >< prod && version =~ "4\.1") BES4Pro = 1;
    ...
    foreach dll (dlls)
    {
      ...
      if (!isnull(fh))
      {
        ...
        if (ver)
        {
          ...
          {
            file_version = ver[0] + "." + ver[1] + "." + ver[2] + "." + ver[3];
...
else exit(0, "The installed " + prod + " version " + version + " is not affected.");
