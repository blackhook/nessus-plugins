...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP1 : xorg-x11-server (EulerOS-SA-2017-1295)");
...
following vulnerability :
  ...
  - In X.Org Server (aka xserver and xorg-server) before
    1.19.4, a local attacker authenticated to the X server
    could overflow a global buffer, causing crashes of the
    X server or potentially other problems by injecting
    large or malformed XKB related atoms and accessing them
    via xkbcomp.(CVE-2017-13723)

Note that Tenable Network Security has extracted the preceding
description block directly from the EulerOS security advisory. Tenable
has attempted to automatically clean and format it as much as possible
without introducing additional issues.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2017-1295
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?4f0bb555");
...
pkgs = ["xorg-x11-server-Xephyr-1.15.0-33.h1",
        "xorg-x11-server-Xorg-1.15.0-33.h1",
        "xorg-x11-server-Xvfb-1.15.0-33.h1",
        "xorg-x11-server-common-1.15.0-33.h1"];
