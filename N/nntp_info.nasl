...
script_end_attributes();
 ...
 script_dependencies("find_service_3digits.nasl", "doublecheck_std_services.nasl", "smtp_settings.nasl");
...
from_addr = get_kb_item('SMTP/headers/From');
if (!from_addr) from_addr = 'Nobody <nobody@example.com>';
local_distrib = "yes";
x_no_archive = "no";
...
more_headers = strcat(
	...
	' was scanned from ', compat::this_host(), '\r\n',
	...
	'X-NNTP-Posting-Host: ', compat::this_host(), '\r\n'	);
...
if (! preg(pattern:"^2[0-9][0-9] ", string:buff)) { nolist=1; }
...
if (!nolist) {
 buff = recv_line(socket:s, length: 2048);
 n = 0;
 while (buff && ! preg(pattern: '^\\.[\r\n]+$', string: buff))
 {
  if (preg(pattern:"^alt\.", string: buff)) { altNB=altNB+1; }
  if (preg(pattern:"^rec\.", string: buff)) { recNB=recNB+1; }
  if (preg(pattern:"^biz\.", string: buff)) { bizNB=bizNB+1; }
  if (preg(pattern:"^sci\.", string: buff)) { sciNB=sciNB+1; }
  if (preg(pattern:"^soc\.", string: buff)) { socNB=socNB+1; }
  if (preg(pattern:"^misc\.", string: buff)) { miscNB=miscNB+1; }
  if (preg(pattern:"^news\.", string: buff)) { altNB=newsNB+1; }
  if (preg(pattern:"^comp\.", string: buff)) { compNB=compNB+1; }
  if (preg(pattern:"^talk\.", string: buff)) { talkNB=talkNB+1; }
  if (preg(pattern:"^humanities\.", string: buff)) { humanitiesNB=humanitiesNB+1; }
  if (preg(pattern:"^microsoft\.", string: buff)) { microsoftNB ++; }

  if (preg(pattern:testRE, string: buff)) {
    ...
    if ((l + 1 + total_len <= 498) && (nbg < max_crosspost)) {
     ...
     nbg = nbg + 1;
     testNGlist = testNGlist + "," + group_name;
    }
  }
 ...
 notice =
    notice + "For your information, we counted " + n +
...
if (! posted && posting)
 notice = notice + "Although this server says it allows posting, we were unable to send a message\n(posted in " + testNGlist + ").\n";
