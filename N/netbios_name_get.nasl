...
if ( get_udp_port_state(port))
{
 ...
 if (!isnull(rep) && max_index(rep) > 1)
 {
  ...
  report =   "The following " + (max_index(rep) - 1) + ' NetBIOS names have been gathered :\n\n';

  for (i=0; i<max_index(rep)-1; i++)
  {
   name = rep[i];
   val = parse_name (name:name);
   desc = get_description (name:val[0], number:val[1], flags:val[2]);

   report += " " + val[0] + crap(data:" ", length:16 - strlen(val[0])) + " = " + desc + '\n';
  }
 ...
 {
   # Additional check for Windows-associated services
   dcerpc_list = NULL;
   dcerpc_list = get_kb_list("Services/dce-rpc");

   epmap_list = NULL;
   epmap_list = get_kb_list("Services/epmap");

   if (empty_or_null(dcerpc_list) && empty_or_null(epmap_list))
   {
     # Windows services not detected.  Probably Samba Server
   ...
   {
     report += '\n' + 'This SMB server is running other Windows services, yet its MAC address is NULL.';
   }
 }
 else
  {
...
if (isnull(report))
{
 ...
 if(! smb_session_init(smb2:TRUE, report_access_trouble:FALSE, report_auth_failure:FALSE)) audit(AUDIT_FN_FAIL, 'smb_session_init');
 ...
 if (!isnull(list))
 {
   ...
   {
    if (domain != nbname) nbgroup = domain;

    report =   'The following 2 NetBIOS names have been gathered :\n\n';
    sp = 16;
    ...
    report += " " + nbname + crap(data:" ", length:sp - strlen(nbname)) + " = " + unique_desc[0x00]+'\n';
    report += " " + domain + crap(data:" ", length:sp - strlen(domain)) + " = " + group_desc[0x00]+'\n';
    set_kb_item(name:"/tmp/10150/report", value: report);
    set_kb_item(name:"/tmp/10150/port", value: port);
...
if (!isnull(nbname))
{
  prev_name = get_kb_item("SMB/name");
  if (!empty_or_null(prev_name))
  {
    spad_log(message:'SMB/name previously determined as ' + prev_name);
    if (prev_name != nbname)
    {
      spad_log(message:"CONFLICT - domain determined by microsoft_windows_nbt_info.nasl: " + prev_name);
      spad_log(message:"CONFLICT - domain determined by netbios_name_get.nasl          : " + nbname);
    }
  }
  else
  {
    spad_log(message:'Setting Netbios SMB/name as ' + nbname);
    set_kb_item(name:"SMB/netbios_name_plugin_source", value:'netbios_name_get.nasl');
  }
