...
if (description)
{
  ...
  script_name(english: "NetSarang Xmanager Enterprise 5 Backdoor Trojan (ShadowPad)");
  script_summary(english:"Checks the Xmanager Enterprise 5 install for a trojaned nssock2.dll file.");

  script_set_attribute(attribute:"synopsis", value:
"The remote host contains an application that is affected by a
trojan backdoor.");
  script_set_attribute(attribute:"description", value:
"The Xmanager Enterprise 5, a network connectivity suite for Windows,
installed on the remote host has a nssock2.dll file identified by its
MD5 hash that is infected with a trojan backdoor.

The affected file includes an encrypted payload that could be
remotely activated by a knowledgeable attacker.");
...
path = get_registry_value(
  handle:hklm,
  item:"SOFTWARE\NetSarang\Xmanager Enterprise\5\Path"
);
