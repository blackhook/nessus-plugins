...
organization.");
  script_set_attribute(attribute:"see_also", value:"https://www.noip.com/");
...
app = "The No-IP Dynamic Update Client";
cpe = "x-cpe:/a:disa:security_readiness_review";
...
foreach key (keys)
{
  ...
  if (!isnull(key_h))
  {
    ...
    if (!isnull(value) && preg(pattern:"DUC[0-9]+\.exe", string:value[1], icase:TRUE))
    {
      exe = value[1];
...
foreach exe (list_uniq(exes))
{
  ...
  register_install(
    vendor:"No-IP",
    product:"Dynamic DNS Update Client",
    app_name:app,
    path:path,
