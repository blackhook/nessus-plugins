...
#
# CHANGELOG:
# v1.3: Only report security warning if NDS users were retrieved
...
if(description)
{
 script_id(10988);
 script_version("1.17");
 script_cvs_date("Date: 2018/10/16 11:11:50");

 script_name(english:"Novell NetWare ncp Service NDS Object Enumeration");
 script_summary(english:"Netware NDS Object Enumeration");
...
to port 524 be blocked from the Internet." );
 ...
 script_set_attribute(attribute:"cvss_score_rationale", value:"An in depth analysis by Tenable researchers revealed the Access Complexity to be low.");
   script_set_attribute(attribute:"plugin_publication_date", value: "2002/06/04");
 ...
 script_copyright(english:"This script is Copyright (C) 2002-2018 Digital Defense, Inc");
	...
	if(soc)
	{
		...
		if("tNcP" >< r)
		{
			...
			if("tNcP" >< r)
			{
				...
				{
					if(ord(r[i]) != 0)
						server_name = server_name + r[i];
				}

				report = "Server Name: " + server_name + "\n";
			}
			...
			if("tNcP" >< r)
			{
				...
				for(i = 24; i < 45; i = i + 1)
				{
					if((r[i] >< "_") && (r[i+1] >< "_"))
					{
						...
						nds_tree_name = nds_tree_name + r[i];
				}

                		report = report + "NDS Tree Name: " + nds_tree_name + "\n";
			}
				...
				if(("tNcP" >< r) && (!(r[14] >< raw_string(0xfc))))
				{
					...
					for(i = 22; i < 71; i = i + 1)
					{
						if(ord(r[i]) == 0)
						{
							...
							nds_object_name = nds_object_name + r[i];
					}

					if(first == 1)
					{
						report_users = report_users + nds_object_name;
						first = 0;
					}
					else
						report_users = report_users + ", " + nds_object_name;
			...
			if(strlen(report) > 0)
			{
				if(strlen(report_users) > 0)
					report = report + "NDS Users: " + report_users;
...
" + report;
				...
				# if users were retrieved mark it as a warning
				if(strlen(report_users) > 0)
