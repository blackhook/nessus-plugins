...
if (description)
{
  ...
  script_name(english: "NetSarang Xlpd 5 Backdoor Trojan (ShadowPad)");
  script_summary(english:"Checks the Xlpd 5 install for a trojaned nssock2.dll file.");

  script_set_attribute(attribute:"synopsis", value:
"The remote host contains an application that is affected by a trojan
backdoor.");
  script_set_attribute(attribute:"description", value:
"The Xlpd 5,a line printer daemon for Windows, installed on the remote
host has a nssock2.dll file identified by its MD5 hash that is
infected with a trojan backdoor.
...
path = get_registry_value(
  handle:hklm,
  item:"SOFTWARE\NetSarang\Xlpd\5\Path"
);
