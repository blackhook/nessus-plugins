...
produced for this port, they might not all be accurate." );
 ...
 script_copyright(english:"This script is Copyright (C) 2000-2022 RD / H D Moore");
 script_family(english:"Web Servers");
 script_dependencies("find_service1.nasl", "httpver.nasl", "http_login.nasl", "webmirror.nasl", "waf_detection.nbin", "broken_web_server.nasl");
...
for (c = 0; badurl[c]; c = c + 1)
{
	...
	if ( egrep(pattern:"^DAAP-Server: ", string:ret) )
	{
	  ...
	  exit(0);
        ...
        # check for a 200 OK
        if(preg(pattern:"^HTTP/[0-9]\.[0-9] 200 ", string:raw_http_line))
        {
...
";
        ...
        three_oh_match = pregmatch(string:raw_http_line, pattern:"^HTTP/[0-9]\.[0-9] (30[12]) ");
