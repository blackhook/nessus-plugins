...
if (description)
{
  ...
  script_name(english: "NetSarang Xmanager 5 Backdoor Trojan (ShadowPad)");
  script_summary(english:"Checks the Xmanager 5 install for a trojaned nssock2.dll file.");

  script_set_attribute(attribute:"synopsis", value:
"The remote host contains an application that is affected by a trojan
backdoor.");
  script_set_attribute(attribute:"description", value:
"The Xmanager 5, an X server for Windows, installed on the remote
...
path = get_registry_value(
  handle:hklm,
  item:"SOFTWARE\NetSarang\Xmanager\5\Path"
);
