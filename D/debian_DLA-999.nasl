...
# extracted from Debian Security Advisory DLA-999-1. The text
...
if (description)
{
  ...
  script_name(english:"Debian DLA-999-1 : openvpn security update");
...
vulnerabilities in openvpn, a popular virtual private network (VPN)
daemon.

If clients used a HTTP proxy with NTLM authentication, a
man-in-the-middle attacker could cause the client to crash or disclose
at most 96 bytes of stack memory, likely to contain the proxy
password.
...
introducing additional issues."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://lists.debian.org/debian-lts-announce/2017/06/msg00028.html"
  );
  script_set_attribute(
    attribute:"see_also",
...
flag = 0;
if (deb_check(release:"7.0", prefix:"openvpn", reference:"2.2.1-8+deb7u5")) flag++;
