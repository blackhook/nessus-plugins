...
# extracted from Debian Security Advisory DSA-3634. The text 
...
if (description)
{
  ...
  script_xref(name:"DSA", value:"3634");

  script_name(english:"Debian DSA-3634-1 : redis - security update");
...
for the updated redis-cli to do so the next time it is run."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=832460"
  );
  script_set_attribute(
    attribute:"see_also",
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://www.debian.org/security/2016/dsa-3634"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
flag = 0;
if (deb_check(release:"8.0", prefix:"redis-server", reference:"2:2.8.17-1+deb8u5")) flag++;
if (deb_check(release:"8.0", prefix:"redis-tools", reference:"2:2.8.17-1+deb8u5")) flag++;
