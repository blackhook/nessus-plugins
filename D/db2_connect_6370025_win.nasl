...
if (description)
{
  ...
  script_name(english:"IBM DB2 Connect 10.5 < FP11 40479 / 11.1 < FP5 40478 / 11.5 < 11.5.5000.1587 Buffer Overflow (Windows)");

  script_set_attribute(attribute:"synopsis", value:
"The remote database server is affected by a buffer overflow vulnerability.");
  script_set_attribute(attribute:"description", value:
"A buffer overflow condition exists in IBM DB2 Connect due to improper bounds checking. An unauthenticated, local
attacker can exploit this to execute arbitrary code on the system with root privileges.
...
get_kb_item_or_exit('SMB/db2_connect/Installed');
...
if (!vuln) audit(AUDIT_INST_PATH_NOT_VULN, app, report_version, path);
