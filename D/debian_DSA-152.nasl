...
# extracted from Debian Security Advisory DSA-152. The text 
...
if (description)
{
  ...
  script_name(english:"Debian DSA-152-1 : l2tpd - missing random seed");
...
affected, since it doesn't contain the l2tpd package."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"http://www.debian.org/security/2002/dsa-152"
  );
  script_set_attribute(attribute:"solution", value:"Upgrade the l2tpd packages.");
...
flag = 0;
if (deb_check(release:"3.0", prefix:"l2tpd", reference:"0.67-1.1")) flag++;
