...
if (description)
{
  ...
  script_set_attribute(attribute:"synopsis", value:"A DCE/RPC service is running on the remote host.");
...
sending an RPC request to the remote port/pipe.");
...
# Other Windows.
rpc_info["2f5f6521-cb55-1059-b446-00df0bce31db"] = "tapsrv|Telephony service|svchost.exe";
rpc_info["367abb81-9844-35f1-ad32-98f038001003"] = "svcctl|Service Control Manager|svchost.exe";

# Others
rpc_info["32d90706-b698-4029-b236-e18ebff582b1"] = "_RPC_|DriverStudio Remote Control (SoftIce)|unknown";
rpc_info["10d1800c-af75-4249-b7a2-484dec69ed3a"] = "_RPC_|DriverStudio Remote Control (SoftIce)|unknown";
...
function Lookup (socket, type, object, interface, handle, entries, fid)
{
 ...
 if (!isnull(fid))
   resp = dce_rpc_pipe_request (fid:fid, code:0x02, data:data);
 else
 {
 ...
 for (i=0 ; i<num_entries; i++)
 {
  ...
  annotation[i] = get_string (blob:substr(resp, pos+28, pos+28+annotation_length-1), pos:0, _type:0);
...
function decode_entry (entry)
{
 ...
 if (protocol == 0x0d)
 {
  ...
  if (!isnull(desc))
  {
   ...
   desc = "Description : Unknown RPC service";

  ret[1] = "UUID : " + tmp + ", version " + ord(part1[17]) + "." + ord(part1[18]) + '\n' + desc;
  ret[2] = tmp;
  return ret;
 }
 ...
 if ((protocol == 0x0a) || (protocol == 0x0b) || (protocol == 0x0c))
 {
  ...
  {
   ret[1] = "Type : Remote RPC service";
  }
  else
   ret[1] = "Type : Local RPC service";
 ...
 {
  ret[1] = "Named pipe : " + get_string (blob:part2, pos:0, _type:0);
  return ret;
 }
 ...
 if (protocol == 0x11)
 {
  ret[1] = "Netbios name : " + get_string (blob:part2, pos:0, _type:0);
  return ret;
 }
 ...
 if (protocol == 0x09)
 {
  tmp = ord(part2[0]) + "." + ord(part2[1]) + "." + ord(part2[2]) + "." + ord(part2[3]);
  ret[1] = "IP : " + tmp;
  ret[2] = tmp;
...
if ( !get_kb_item("Host/scanned") || get_port_state(port) )
{
 ...
 if (soc)
 {
  ...
  else
  {
   ...
   {
    register_service(port:135, proto:"epmap");

    data = raw_dword (d:0)        + # Inquiry type
           raw_dword (d:0)        + # Object
           raw_dword (d:0)        + # interface
           raw_dword (d:0)        + # version option
           crap(data:raw_string(0), length:20) + # handle
           raw_dword (d:1)  ; # Max entries
...
if (!rawtcp)
{
 ...
 fid = bind_pipe (pipe:"\epmapper", uuid:"e1af8308-5d1f-11c9-91a4-08002b14a0fa", vers:3);
 if (isnull(fid))
 {
  NetUseDel();
  exit(0);
...
while (!end)
{
 values = Lookup (socket:soc, type:0, object:NULL, interface:NULL, handle:handle, entries:10, fid:fid);
 if (!isnull(values))
 {
  k++;
  handle = values[0];
  ...
  for (i=1; i<max_index(values); i++)
  {
   ...
   if (!isnull(ret))
   {
    if (max_index(ret) >= 6)
    {
     ...
     {
      if (!isnull(ret[1]))
        desc = "Annotation : " + ret[1] + '\n';
      else
        desc = NULL;

      report = "Object UUID : " + ret[0] + '\n' + entry1[1] + '\n' +
               desc + entry3[1] + '\n'+ entry4[1] + '\n';
      if (max_index(ret) > 6)
      {
      ...
      if (report)
      {
       ...
       if (entry4[0] == 0x07 || entry4[0] == 0x08)
       {
        if (entry4[0] == 0x07)
        {
         register_service(port:entry4[2], proto:"DCE/" + entry1[2]);
         set_kb_item (name:"DCE/" + entry1[2] + "/context_handle", value:ret[0]);

         remote_tcp_ports_report[entry4[2]] += report + '\n';
        }
        else
        {
          register_service(port:entry4[2], proto:"DCE/" + entry1[2], ipproto: "udp");
          remote_udp_ports_report[entry4[2]] += report + '\n';
        }
       }
       else
       {
...
{
 foreach var dceport (keys(remote_tcp_ports_report))
 {
  report = '\n'+
           "The following DCERPC services are available on TCP port " +
           dceport + ' :\n\n' + remote_tcp_ports_report[dceport];

  register_service(port:int(dceport), proto:"dce-rpc");
  security_note (port:dceport, extra:report);
 }
}
...
if (!isnull(remote_udp_ports_report))
{
 ...
 {
  report = '\n' +
           "The following DCERPC services are available on UDP port " +
           dceport + ' :\n\n' + remote_udp_ports_report[dceport];

  register_service(port:int(dceport), proto:"dce-rpc", ipproto:"udp");
  security_note (port:dceport, extra:report, proto:"udp");
 }
}
