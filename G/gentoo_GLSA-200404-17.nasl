...
# extracted from Gentoo Linux Security Advisory GLSA 200404-17.
#
...
Workaround :
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"http://ipsec-tools.sourceforge.net/"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/200404-17"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
"ipsec-tools users should upgrade to version 0.2.5 or later:
  ...
  script_set_attribute(attribute:"patch_publication_date", value:"2004/04/24");
...
if (ourarch !~ "^(amd64|ppc|ppc64|s390)$") audit(AUDIT_ARCH_NOT, "amd64|ppc|ppc64|s390", ourarch);

flag = 0;

if (qpkg_check(package:"net-misc/iputils", arch:"ppc amd64 ppc64 s390", unaffected:make_list("eq 021109-r3"), vulnerable:make_list("eq 021109-r1"))) flag++;
if (qpkg_check(package:"net-firewall/ipsec-tools", arch:"amd64", unaffected:make_list("ge 0.3.1"), vulnerable:make_list("lt 0.3.1"))) flag++;
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, "net-misc/iputils / net-firewall/ipsec-tools");
}
