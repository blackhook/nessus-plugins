...
# extracted from Gentoo Linux Security Advisory GLSA 200807-12.
#
...
patches."
  ...
  script_set_attribute(
    ...
    value:
"The remote host is affected by the vulnerability described in GLSA-200807-12
...
Workaround :
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/200807-12"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
"Since BitchX is no longer maintained, we recommend that users unmerge
  ...
  script_set_attribute(attribute:"cpe", value:"p-cpe:/a:gentoo:linux:bitchx");
...
if (qpkg_check(package:"net-irc/bitchx", unaffected:make_list(), vulnerable:make_list("le 1.1-r4"))) flag++;
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, "BitchX");
}
