...
# extracted from Gentoo Linux Security Advisory GLSA 201701-20.
#
...
if (description)
{
  ...
  script_xref(name:"GLSA", value:"201701-20");

  script_name(english:"GLSA-201701-20 : D-Bus: Format string vulnerability");
...
patches."
  ...
  script_set_attribute(
    ...
    value:
"The remote host is affected by the vulnerability described in GLSA-201701-20
...
Workaround :
    ...
    The administrator can install a policy which denies sending from
      org.freedesktop.systemd1.Activator&rdquo; to D-Bus. This will prevent
      non-root attackers from reaching the interface in order to exercise this
      flaw."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/201503-02"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/201701-20"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
if (qpkg_check(package:"sys-apps/dbus", unaffected:make_list("ge 1.10.12"), vulnerable:make_list("lt 1.10.12"))) flag++;
