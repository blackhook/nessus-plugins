...
# extracted from Gentoo Linux Security Advisory GLSA 201611-07.
#
...
if (description)
{
  ...
  script_xref(name:"GLSA", value:"201611-07");

  script_name(english:"GLSA-201611-07 : polkit: Heap-corruption on duplicate IDs");
...
patches."
  ...
  script_set_attribute(
    ...
    value:
"The remote host is affected by the vulnerability described in GLSA-201611-07
(polkit: Heap-corruption on duplicate IDs)

    A vulnerability was discovered in polkit&rsquo;s
...
Workaround :
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/201611-07"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
if (qpkg_check(package:"sys-auth/polkit", unaffected:make_list("ge 0.113"), vulnerable:make_list("lt 0.113"))) flag++;
