...
# extracted from Gentoo Linux Security Advisory GLSA 200612-19.
#
...
patches."
  ...
  script_set_attribute(
    ...
    value:
"The remote host is affected by the vulnerability described in GLSA-200612-19
...
Workaround :
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://security.gentoo.org/glsa/200612-19"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
if (qpkg_check(package:"sys-auth/pam_ldap", unaffected:make_list("ge 183"), vulnerable:make_list("lt 183"))) flag++;
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, "pam_ldap");
}
