...
"Upgrade to phpMyFAQ version 1.4.7 or 1.5.0 RC2 or greater.");
  ...
  script_dependencies("phpmyfaq_detect.nasl", "smtp_settings.nasl");
...
# Test an install.
install = get_kb_item("www/" + port + "/phpmyfaq");
...
if (!isnull(matches)) {
  ...
  if (safe_checks()) {
    ...
    r = http_send_recv3(method:"GET", item:dir + "/index.php?action=open", port:port);
    if (isnull(r)) exit(0);
    res = r[2];
    email = str_replace(string:email, find:"@", replace:"_AT_");
    email = str_replace(string:email, find:".", replace:"_DOT_");
    if (egrep(string:res, pattern:'1970.*<br.+ href="mailto:' + email))
    {
      security_hole(port);
      set_kb_item(name: 'www/'+port+'/SQLInjection', value: TRUE);
    }
