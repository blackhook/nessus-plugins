...
if (description)
{
  ...
  script_name(english:"Investintech Able2Doc Detection");
  script_summary(english:"Checks for install of Able2Doc");

  script_set_attribute(attribute:"synopsis", value:"The remote host has a PDF converting application installed.");
  script_set_attribute(
    attribute:"description",
    value:
"The remote host has an install of Investintech Able2Doc, which is a PDF
converter."
  );
  script_set_attribute(attribute:"see_also", value:"https://www.investintech.com/prod_a2d.htm");
...
{
  if ('Able2Doc' >< key) in_registry = TRUE;
}
...
foreach key (keys(display_names))
{
  ...
  if ("Able2Doc" >< display_name)
  {
    ...
    if ("Professional" >< display_name) paths[path] = 'Able2DocPro.exe';
    else paths[path] = 'Able2Doc.exe';
  }
}
...
foreach path (keys(paths))
{
  ...
  register_install(
    ...
    cpe:"cpe:/a:investintech:able2doc");
