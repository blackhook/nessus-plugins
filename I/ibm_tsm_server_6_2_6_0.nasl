...
if (description)
{
  ...
  script_name(english:"IBM Tivoli Storage Manager Server 6.2.x < 6.2.6.0 Multiple Vulnerabilities");
...
is 6.2.x prior to 6.2.6.0. It is, therefore, potentially affected by
multiple flaws in its bundled SSL library:
...
# We are concerned specifically with 6.2
if(version !~ "^6\.2(\.|$)") audit(AUDIT_NOT_LISTEN, prod+" 6.2", port);
