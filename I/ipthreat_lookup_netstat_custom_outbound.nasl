...
if (description)
{
  ...
  script_name(english:"Active Outbound Connection From Host Listed in Custom Netstat IP Threat List");
  script_summary(english:"Uses results of nbin to report outbound custom ipthreat connections.");

  script_set_attribute(attribute:"synopsis", value:
"According to a custom netstat IP threat list, the remote host is
making an  outbound connection to a host that is listed as a threat.");
...
investigate further if necessary.");
  ...
  script_require_keys("ipthreat/lookup/custom/outbound");
...
report = get_kb_item_or_exit("ipthreat/lookup/custom/outbound");
