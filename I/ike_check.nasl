...
# Changes by Tenable:
# - Revised plugin title, formatted description block (6/17/09)
...
if(description)
{
 ...
 script_name(english:"IPSEC Server ISAKMP Implementation Malformed IKE Request Remote DoS");

 script_set_attribute(attribute:"synopsis", value:
"The remote IPSEC server is affected by a remote denial of service
vulnerability." );
 script_set_attribute(attribute:"description", value:
"The remote IPSEC server seems to have a problem negotiating malformed
IKE requests.  An attacker may use this flaw to disable your VPN
remotely.

Note that this plugin does not run over IPv6.");
 script_set_attribute(attribute:"see_also", value:"https://tools.ietf.org/html/rfc2409" );
...
script_end_attributes();
 ...
 script_summary(english:"IPSEC IKE check");
 script_category(ACT_KILL_HOST);
 script_copyright(english:"This script is Copyright (C) 2002-2019 John Lampe...j_lampe@bellsouth.net");
 script_family(english:"Denial of Service");
 script_require_keys("Settings/ParanoidReport");
 script_dependencies("ike2_detect.nasl", "ike1_aggressive_mode_with_psk.nasl"); # prevent ike_check.nasl from receiving responses destinated to other IKE plugins
 exit(0);
}
...
function bada_bing (blat) {
  local_var ip, result_suc, UDP_LEN, udpip;
...
filter = string("udp and src host ", get_host_ip(), " and dst host " , compat::this_host(), " and dst port 500 and src port 500");
...
alive = end_denial();
if(!alive) security_warning(port:500, protocol:"udp");
...
if (live) {
    ...
    if (protocol_type == IPPROTO_ICMP) {
        security_warning(port:500, protocol:"udp");
    }
}
