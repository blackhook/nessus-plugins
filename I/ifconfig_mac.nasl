...
var devs = make_array();    # for building a string
var unique_macs = make_array();
var ordered_macs = make_list();

var addr, addr_parts, aliased, cur_len, dev, dev_str, i, iface_name, ifconfig, info, info_str;
var lanscan, lanscan_pat, line, mac, mac_addr, mac_bytes, match, netstat, netstat_pat, nwmgr, nwmgr_regex;
var pat_dev, pat_mac, physical, report, res, s, sep, shortest, target_ip, target_mac, temp_mac, tgt_str;
...
else if ('HP-UX' >< uname)
{
  ...
  {
    match = pregmatch(pattern:lanscan_pat, string:line);
    if (!isnull(match))
    {
      temp_mac = '';
      mac_addr = match[1];
...
foreach mac (unique_macs)
{
  ...
  for (i = 0; i < len(devs[mac]); i++)
  {
    ...
    # set 'virtual' kb item for virtual or MACVLAN interfaces
    if (i != physical || '.' >< iface_name)
...
if (empty_or_null(info))
  exit(0, 'Unable to parse any MAC addresses.');

info = join(info,sep:'\n') + '\n';

ordered_macs = join(ordered_macs,sep:'\n');
if (!empty_or_null(ordered_macs))
{
