...
if (description)
{
  ...
  script_xref(name:"IAVT", value:"0001-T-0681");
...
if (ooo && !isnull(buildid) && !isnull(path))
{
  ...
  vers[9790] = "4.1.6 (Revision 1844436)";
  vers[9789] = "4.1.5 (Revision 1817496)";
  vers[9788] = "4.1.4 (Revision 1811857)";
  vers[9783] = "4.1.3";
  ...
  ver = "build " + buildid;
  matches = pregmatch(pattern:"([0-9]+[a-z][0-9]+)\(Build:([0-9]+)\)", string:buildid);
  if (!isnull(matches))
  {
    prod = matches[1];
    build = int(matches[2]);
