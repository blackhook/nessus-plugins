...
if (description)
{
  ...
  script_name(english:"OpenSSL 1.0.1 < 1.0.1c TLS/DTLS CBC Denial of Service");
...
of OpenSSL 1.0.1 earlier than 1.0.1c.  As such, the OpenSSL library
...
function 'dtls1_enc'. When in CBC mode, TLS/DTLS record length values and
explicit initialization vector length values related to TLS/DTLS
packets are not handled properly, which can lead to memory corruption and
application crashes."
  );
  ...
  script_set_attribute(attribute:"see_also", value:"http://cvs.openssl.org/chngview?cn=22547");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=820686");
  script_set_attribute(attribute:"solution", value:"Upgrade to OpenSSL 1.0.1c or later.");
...
openssl_check_version(fixed:'1.0.1c', min:"1.0.1", severity:SECURITY_WARNING);
