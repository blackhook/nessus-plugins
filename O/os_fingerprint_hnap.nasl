...
for (i=0; i<n; i++)
{
  if (
    (
      !vendor_pat[i] ||
      (vendor && pregmatch(pattern:vendor_pat[i], string:vendor))
    ) &&
    (
      !model_pat[i] ||
      (model && pregmatch(pattern:model_pat[i], string:model))
    )
  )
  {
    name = name[i];
    ...
    {
      model_match = pregmatch(pattern:model_pat[i], string:model);
