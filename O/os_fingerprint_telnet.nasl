...
function check_banner()
{
  ...
  {
    line = pgrep(pattern:"SunOS 5\.[0-9]", string:banner);
    if (line && line != banner)
    {
      os = "Solaris";
  ...
  else if ("FreeBSD/" >< banner)
  {
    ...
    pgrep(pattern:"Serial Number .* MAC address", string:banner) &&
  ...
  else if ("QNX Neutrino (" >< banner)
  {
    ...
    pgrep(pattern:"^Corel Linux/", string:banner) ||
    eregmatch(pattern:"^(Linux )?Kernel ([12]\.[0-6](\.[0-9A-Za-z.-]+))", string:chomp(banner)) ||
    pgrep(pattern:"Kernel [0-9.\-]+\.fc\d\d\.(i686|i686\+PAE|x86_64) ", string:banner)
  )
  {
    ...
    if ("Ubuntu " >< banner)
    {
      # Ubuntu 8.04.1
      # ubuntu login:
      line = pgrep(pattern:"^Ubuntu ([4-9]|[1-9][0-9])\.([0-9]{2})(\.[0-9])? *$", string:banner);
      if (strlen(line) > 0)
      {
    ...
    else if ("CentOS " >< banner)
    {
      ...
      line = pgrep(pattern:"^CentOS( Linux)? release [1-6]\.[0-9] ", string:banner);
      if (strlen(line) > 0)
      {
        line = chomp(line);
    ...
    {
      line = pgrep(pattern:"Kernel [0-9.\-]+\.vz\d\.[0-9.]+ on an (i686|i686\+PAE|x86_64)", string:banner);
    ...
    )
    {
      ...
      # login:
      line = pgrep(pattern:"^Fedora (Core )?release [1-9][0-9]* ", string:banner);
      if (strlen(line) > 0)
      {
        line = chomp(line);
        if (!linux_kernel)
      ...
      else if (pgrep(pattern:"Kernel [0-9.\-]+\.fc\d\d\.(i686|i686\+PAE|x86_64) ", string:banner))
      {
        line = pgrep(pattern:"Kernel [0-9.\-]+\.fc\d\d\.(i686|i686\+PAE|x86_64) ", string:banner);
        if (strlen(line) > 0)
        {
          line = chomp(line);
          if (!linux_kernel)
    ...
    else if ("Red Hat Enterprise Linux " >< banner)
    {
      ...
      line = pgrep(pattern:pat, string:banner);
      if (strlen(line) > 0)
      {
        line = chomp(line);
        ...
        if (!isnull(match))
        {
          if (!linux_kernel)
          {
            if (pgrep(pattern:" release 7($|[^0-9])", string:banner)) linux_kernel = "Linux Kernel 3.10";
    ...
    else if ("Red Hat Linux " >< banner)
    {
      ...
      line = pgrep(string: banner, pattern: "^Red Hat Linux release [0-9.]+ " );
      if (strlen(line) > 0)
      {
        line = chomp(line);
        if (! linux_kernel)
    ...
    else if ("Linux Mandrake" >< banner)
    {
      ...
      line = pgrep(string: banner, pattern:"^Linux Mandrake release [5-9]\.[0-9]+ ");
      if (strlen(line) > 0)
      {
    ...
    else if ("Debian" >< banner)
    {
      ...
      line = pgrep(string: banner, pattern: "^Debian GNU/Linux [1-9]\.[0-9]");
      if (strlen(line) > 0)
      {
    ...
    else if ("SUSE" >< banner || "SuSE" >< banner)
    {
      ...
      line = pgrep(pattern:'^Welcome to (open)?S[uU]SE ([A-Z][a-z]+ )*[0-9.]+( .*[\\("].*[\\)"])? - Kernel [0-9]', string:banner);
      match = NULL;
    ...
    else if (pgrep(pattern:"^Corel Linux/", string:banner))
    {
    ...
    else if ("Caldera OpenLinux(TM)" >< banner)
    {
      ...
      line = pgrep(pattern:"^Version +[12]\.[0-9]+", string:banner);
      if (strlen(line) > 0)
      {
  ...
  else if ("MikroTik v" >< banner)
  {
    ...
    # MikroTik v3.2
    # Login:
    line = pgrep(string: banner, pattern:"^MikroTik v[0-9][0-9.]+");
    if (line && line != banner)
    {
  ...
  else if (pgrep(pattern:"^Service Processor login:", string:banner))
  {
    ...
    pgrep(pattern:"ProCurve [^ ]+ Switch", string:banner)
  )
  {
    ...
    (
      ...
      pgrep(pattern:"MPS-[^ ]+ Control Console", string:banner)
    )
  )
  {
    ...
    pgrep(pattern:"Lantronix (MSS|UDS)", string:banner)
  )
  {
  ...
  {
    res["os"] = 'VxWorks';
    res["confidence"] = 71;     # nb: keep low as VxWorks is used in a lot of embedded kit.
    res["type"] = "embedded";
    ...
    pgrep(pattern:"AP-[^ ]+ Access Point [0-9]", string:banner)
  )
  {
  ...
  )
  {
    ...
    match = pgrep(pattern:"^TANDBERG Codec Release .+", string:banner);
    if (match)
    {
  ...
  )
  {
    ...
    "Fabric OS" >< banner &&
    pgrep(pattern:"(Fabos Version |Release v)[0-9]", string:banner)
  )
  {
    os = "Brocade Switch";
  ...
  )
  {
    ...
    (
      ' 4690 OS ' >< banner &&
      pgrep(pattern:"Type.*your.+Operator.+ID", string:banner) &&
      pgrep(pattern:"Copyright.+(IBM|Toshiba)", string:banner)
    )
  )
  {
    os = "Toshiba 4690 OS";
  ...
  else if (pgrep(pattern:" SHARP (AR|MX)-.+ TELNET server", string:banner))
  {
  ...
  else if (pgrep(pattern:"/ ScreenOS [0-9.]+", string:banner) )
  {
    match = eregmatch(pattern:".* / ScreenOS ([0-9.a-z][^ ]+) .*", string:banner);
    
    if (!empty_or_null(match) && match[1])
  ...
  else if (pgrep(pattern:"AXIS [0-9][^ ]+ TELNET Network Print Server", string:banner))
  {
  ...
  else if (pgrep(pattern:"Meinberg LANTIME OS6 LOGIN", string:banner) )
  {
  ...
  else if('Grandstream' >< banner)
  {
    res['os'] = 'Grandstream VoIP Phone';
    res['confidence'] = 80;
    res['type'] = 'telephony';

    return res;
  }
  else if('FUJITSU FLASHWAVE' >< banner)
  {
    res['os'] = 'Fujitsu FLASHWAVE';
    res['confidence'] = 75;
    res['type'] = 'packet-onp';     # Packet Optical Networking Platform
...
best_res = NULL;
foreach var port (make_list(ports))
{
