...
{
  os = "Solaris";
  match = pregmatch(pattern:" FTP server \(SunOS 5\.([0-9]+)", string:banner);
  if (!isnull(match))
  {
    version = match[1];
...
{
  os = "VxWorks";
  match = pregmatch(pattern:" VxWorks \((VxWorks *)?([0-9][0-9.]+)\) FTP server", string:banner);
  if (!isnull(match)) os += " " + match[2];

  set_kb_item(name:"Host/OS/FTP", value:os);
  set_kb_item(name:"Host/OS/FTP/Type", value:"embedded");
  set_kb_item(name:"Host/OS/FTP/Confidence", value:72);     # nb: keep low as VxWorks is used in a lot of embedded kit.
  exit(0);
}
...
if ("IBM FTP CS" >< banner)
{
  os = "IBM z/OS";
  match = pregmatch(pattern:"IBM FTP CS V([0-9]+)R([0-9]+)", string:banner);
...
{
  match = pregmatch(pattern:" AXIS ([0-9][^ ]+) FTP Network Print Server .+ ready", string:banner);
...
{
  match = pregmatch(pattern:" (AXIS|Axis) ([A-Z]?[0-9][^ ]+)( (Fixed Dome|PTZ))? Network Camera ([0-9][0-9.]+ \(.+\)) ready", string:banner);
...
if (" FTP server (MikroTik " >< banner)
{
  ...
  match = pregmatch(pattern:" FTP server \(MikroTik ([0-9][0-9.]+)\)", string:banner);
...
{
  # 220 FTP server (NetApp Release 7.2.1S10R1: Thu Apr 16 23:01:18 PDT 2009) ready
  # 220 FTP server (NetApp Release 7.3.1P3: Thu Apr  2 13:29:49 PDT 2009) ready
  # 220 FTP server (NetApp Release 7.3.3: Thu Mar 11 22:43:52 PST 2010) ready
  # 220 FTP server (NetApp Release 7.3.5.1: Sat Jan 29 12:45:56 PST 2011) ready
  # 220 FTP server (NetApp Release 8.1.4 7-Mode: Wed Nov 20 16:16:17 PST 2013) ready
  # 220 FTP server (NetApp Release 8.1.4P2 7-Mode: Sun Apr 13 00:25:56 PDT 2014) ready
  # 220 FTP server (NetApp Release 8.1.4P3 7-Mode: Fri May 16 23:15:17 PDT 2014) ready
  # 220 FTP server (NetApp Release 8.1.4P8 7-Mode: Wed Feb  4 02:08:52 PST 2015) ready
  # 220 FTP server (NetApp Release 8.2.2RC2 7-Mode: Tue Jun 24 19:34:55 PDT 2014) ready
  match = pregmatch(string:banner, pattern:"FTP server \(NetApp Release ([0-9][^\s:]+)" );
  if (!isnull(match))
  {
...
{
  match = pregmatch(pattern:" SHARP ((AR|MX)-[^ ]+) Ver ([0-9][^ ]+) FTP server", string:banner);
  if (isnull(match))
  {
    os = "Sharp Printer";
    conf = 80;
...
{
  os = "AsyncOS";
  match = pregmatch(string:banner, pattern:"Cisco IronPort FTP server \(V([0-9][^ )]+)\) ready");
...
if ("FTP server (StarOS)" >< banner)
{
  os = "Cisco StarOS";
  
  set_kb_item(name:"Host/OS/FTP", value:os);
  set_kb_item(name:"Host/OS/FTP/Type", value:"embedded");
  set_kb_item(name:"Host/OS/FTP/Confidence", value:71);  # beat SinFP banner.
  exit(0);
}
...
{
  os = "GuardianOS";
  match = pregmatch(string:banner, pattern:"GuardianOS v([0-9.]+)");
  if (!isnull(match)) {
...
if (pgrep(pattern:"Isilon OneFS v[0-9][0-9.]+$",string:banner))
{
  os = "Isilon OneFS";
