...
if (description)
{
  ...
  script_name(english:"Oracle Linux 6 : Oracle / Linux / 6 / kernel (ELSA-2013-1645)");
...
ELSA-2013-1645 advisory.
  ...
  - Format string vulnerability in the register_disk function in block/genhd.c in the Linux kernel through
    3.9.4 allows local users to gain privileges by leveraging root access and writing format string specifiers
    to /sys/module/md_mod/parameters/new_array in order to create a crafted /dev/md device name.
    (CVE-2013-2851)
  ...
  - net/ipv6/ip6_output.c in the Linux kernel through 3.11.4 does not properly determine the need for UDP
    Fragmentation Offload (UFO) processing of small packets after the UFO queueing of a large packet, which
    allows remote attackers to cause a denial of service (memory corruption and system crash) or possibly have
    unspecified other impact via network traffic that triggers a large response packet. (CVE-2013-4387)
...
var pkgs = [
    ...
    {'reference':'kernel-abi-whitelists-2.6.32-431.el6', 'release':'6', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'kernel-abi-whitelists-2.6.32'},
    ...
    {'reference':'kernel-firmware-2.6.32-431.el6', 'release':'6', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'kernel-firmware-2.6.32'},
