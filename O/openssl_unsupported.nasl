...
if (description)
{
  script_id(78555);
  script_version("1.10");
  script_set_attribute(attribute:"plugin_modification_date", value:"2021/02/15");
...
latest_supported_versions = "1.0.2 / 1.1.1 / 3.0.0";
...
# branch check
matches = pregmatch(pattern:"^([\d.]+)", string:version);
if (empty_or_null(matches)) audit(AUDIT_SERVICE_VER_FAIL, service_name, port);
numeric_version = matches[1];

if (ver_compare(ver:numeric_version, fix:lowest_supported_branch, strict:FALSE) == -1 ||  numeric_version =~ "^1.1.0([^0-9]|$)")
{
