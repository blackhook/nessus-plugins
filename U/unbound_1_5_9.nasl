...
if (description)
{
  ...
  script_name(english:"Unbound < 1.5.9 daemon/remote.c Diffie-Hellman Key Exchange Value Small Subgroup Confinement Attack Vulnerability");
  script_summary(english:"Checks version of Unbound");

  script_set_attribute(attribute:"synopsis", value:
"The remote name server is affected by a key confinement attack
vulnerability.");
  script_set_attribute(attribute:"description", value:
"According to its self-reported version number, the remote Unbound DNS
resolver contains a flaw in the handling of Diffie-Hellman key
exchange values that can allow a remote attacker to perform a small
subgroup confinement attack.");
  script_set_attribute(attribute:"see_also", value:"https://nlnetlabs.nl/projects/unbound/download/");
  script_set_attribute(attribute:"see_also", value:"https://www.nlnetlabs.nl/bugs-script/show_bug.cgi?id=770");
  script_set_attribute(attribute:"see_also", value:"https://en.wikipedia.org/wiki/Small_subgroup_confinement_attack");

  script_set_attribute(attribute:"solution", value:"Upgrade to Unbound version 1.5.9 or later.");
...
# if version < 1.5.9 (including patches and rc)
if (
  version =~ "^0\." ||
  version =~ "^1\.[0-4]($|[^0-9])" ||
  version =~ "^1\.5(\.[0-8](\.[0-9]+)*)?(([abp]|rc)[0-9]*)?$" ||
  version =~ "^1\.5\.9([ab]|rc)[0-9]*$"
)
{
  report =
