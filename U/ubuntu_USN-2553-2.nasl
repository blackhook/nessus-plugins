...
if (description)
{
  ...
  script_name(english:"Ubuntu 10.04 LTS / 12.04 LTS / 14.04 LTS / 14.10 : tiff regression (USN-2553-2)");
...
caused a regression when saving certain TIFF files with a Predictor
tag. The problematic patch has been temporarily backed out until a
...
Paris Zoumpouloglou discovered that LibTIFF incorrectly
handled certain malformed BMP images. If a user or automated
system were tricked into opening a specially crafted BMP
image, a remote attacker could crash the application,
leading to a denial of service. (CVE-2014-9330)

Michal Zalewski discovered that LibTIFF incorrectly handled
certain malformed images. If a user or automated system were
tricked into opening a specially crafted image, a remote
attacker could crash the application, leading to a denial of
service, or possibly execute arbitrary code with user
privileges. (CVE-2014-9655).
...
without introducing additional issues."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://usn.ubuntu.com/2553-2/"
  );
  script_set_attribute(
    attribute:"solution", 
...
if (ubuntu_check(osver:"10.04", pkgname:"libtiff4", pkgver:"3.9.2-2ubuntu0.16")) flag++;
if (ubuntu_check(osver:"12.04", pkgname:"libtiff4", pkgver:"3.9.5-2ubuntu1.8")) flag++;
if (ubuntu_check(osver:"14.04", pkgname:"libtiff5", pkgver:"4.0.3-7ubuntu0.3")) flag++;
if (ubuntu_check(osver:"14.10", pkgname:"libtiff5", pkgver:"4.0.3-10ubuntu0.2")) flag++;
