...
{
  if (! preg(pattern:"^(4.15.0-\d{3}-(generic|lowlatency)|4.15.0-\d{4}-oracle)$", string:machine_kernel_release)) audit(AUDIT_INST_VER_NOT_VULN, 'kernel ' + machine_kernel_release);
  var extra = '';
  var kernel_mappings = {
    "4.15.0-\d{3}-(generic|lowlatency)" : "4.15.0-194",
