...
{
  if (! preg(pattern:"^(4.4.0-\d{3}-(generic|lowlatency)|4.4.0-\d{4}-kvm)$", string:machine_kernel_release)) audit(AUDIT_INST_VER_NOT_VULN, 'kernel ' + machine_kernel_release);
  var extra = '';
  var kernel_mappings = {
    "4.4.0-\d{3}-(generic|lowlatency)" : "4.4.0-236",
    "4.4.0-\d{4}-kvm" : "4.4.0-1116"
  };
