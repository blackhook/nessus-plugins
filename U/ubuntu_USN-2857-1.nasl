...
# extracted from Ubuntu Security Notice USN-2857-1. The text 
...
if (description)
{
  ...
  script_xref(name:"USN", value:"2857-1");

  script_name(english:"Ubuntu 15.04 : linux vulnerability (USN-2857-1)");
...
patches."
  ...
  script_set_attribute(
    ...
    value:
"Nathan Williams discovered that overlayfs in the Linux kernel
incorrectly handled setattr operations. A local unprivileged attacker
could use this to create files with administrative permission
attributes and execute arbitrary code with elevated privileges.
...
without introducing additional issues."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://usn.ubuntu.com/2857-1/"
  );
  script_set_attribute(
    attribute:"solution", 
    value:
...
if (get_one_kb_item("Host/ksplice/kernel-cves"))
{
  ...
  cve_list = make_list("CVE-2015-8660");
  if (ksplice_cves_check(cve_list))
  {
    audit(AUDIT_PATCH_INSTALLED, "KSplice hotfix for USN-2857-1");
  }
  else
  {
...
if (ubuntu_check(osver:"15.04", pkgname:"linux-image-3.19.0-43-generic", pkgver:"3.19.0-43.49")) flag++;
if (ubuntu_check(osver:"15.04", pkgname:"linux-image-3.19.0-43-generic-lpae", pkgver:"3.19.0-43.49")) flag++;
if (ubuntu_check(osver:"15.04", pkgname:"linux-image-3.19.0-43-lowlatency", pkgver:"3.19.0-43.49")) flag++;
