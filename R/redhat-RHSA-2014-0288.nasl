...
if (description)
{
  ...
  script_name(english:"RHEL 4 / 5 / 6 : gnutls (RHSA-2014:0288)");
...
applications linked to the GnuTLS library must be restarted."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2014:0288"
  );
  script_set_attribute(
    attribute:"see_also",
...
os_ver = os_ver[1];
if (! preg(pattern:"^(4|5\.3|5\.6|5\.9|6)([^0-9]|$)", string:os_ver)) audit(AUDIT_OS_NOT, "Red Hat 4.x / 5.3 / 5.6 / 5.9 / 6.x", "Red Hat " + os_ver);
...
{  sp = get_kb_item("Host/RedHat/minor_release");
  ...
  flag = 0;
  if (rpm_check(release:"RHEL4", cpu:"i386", reference:"gnutls-1.0.20-5.el4")) flag++;

  if (rpm_check(release:"RHEL4", cpu:"x86_64", reference:"gnutls-1.0.20-5.el4")) flag++;

  if (rpm_check(release:"RHEL4", cpu:"i386", reference:"gnutls-devel-1.0.20-5.el4")) flag++;

  if (rpm_check(release:"RHEL4", cpu:"x86_64", reference:"gnutls-devel-1.0.20-5.el4")) flag++;
