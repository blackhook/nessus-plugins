...
Ansible: Splunk and Sumologic callback plugins leak sensitive data in
logs (CVE-2019-14864)
...
See:
https://github.com/ansible/ansible/blob/v2.9.1/changelogs/CHANGELOG-v2
.9.rst for details on bug fixes in this release."
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2019:3928"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/security/cve/cve-2019-14864"
  );
  script_set_attribute(
    attribute:"solution", 
...
else
{
  ...
  if (! (rpm_exists(release:"RHEL7", rpm:"ansible-2.[0-6]") || rpm_exists(release:"RHEL8", rpm:"ansible-2.[0-6]"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "Red Hat Ansible 2.0 - 2.6");

  if (rpm_check(release:"RHEL7", reference:"ansible-2.9.1-1.el7")) flag++;
  if (rpm_check(release:"RHEL7", reference:"ansible-test-2.9.1-1.el7")) flag++;

  if (rpm_check(release:"RHEL8", reference:"ansible-2.9.1-1.el8")) flag++;
  if (rpm_check(release:"RHEL8", reference:"ansible-test-2.9.1-1.el8")) flag++;
