...
if (description)
{
  ...
  script_set_attribute(
    ...
    value:
"The updated grafana package is now available for Red Hat Ceph Storage
3.2.
...
* The grafana package has been upgraded to upstream version 5.2.4.,
which includes a number of bug fixes (BZ#1647494)

* Shrinking the cluster size no longer causes the Red Hat Ceph Storage
Dashboard to display the error message Templating init failed
(BZ#1653273)"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2019:0019"
  );
  script_set_attribute(
    attribute:"see_also",
...
else
{
  ...
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"grafana-5.2.4-1.el7")) flag++;
