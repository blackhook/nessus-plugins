...
if (description)
{
  ...
  script_name(english:"RHEL 7 : qemu-kvm-rhev (RHSA-2018:2060) (Spectre)");
...
that includes support for guests running on hosts with AMD processors.

Bug Fix(es) :

* Previously, using device passthrough for a SCSI-2 device failed and
returned an 'Illegal Request' error. With this update, the QEMU
emulator checks the SCSI version of the device when performing
passthrough. As a result, the described problem no longer occurs.
(BZ#1571370)

* Under certain circumstances, resuming a paused guest generated
redundant 'VIR_DOMAIN_PAUSED_UNKNOWN' error messages in the libvirt
log. This update corrects the event sending order when resuming
guests, which prevents the errors being logged. (BZ#1582122)

Enhancement(s) :

* With this update, Ceph storage is supported by KVM virtualization on
all CPU architectures supported by Red Hat. (BZ#1588001)"
  );
  script_set_attribute(
    attribute:"see_also",
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2018:2060"
  );
  script_set_attribute(
    attribute:"see_also",
...
else
{
  ...
  if (! (rpm_exists(release:"RHEL7", rpm:"qemu-kvm-rhev-2.10.0"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "Red Hat Virtualization 4");

  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"qemu-img-rhev-2.10.0-21.el7_5.4")) flag++;
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"qemu-kvm-common-rhev-2.10.0-21.el7_5.4")) flag++;
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"qemu-kvm-rhev-2.10.0-21.el7_5.4")) flag++;
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"qemu-kvm-rhev-debuginfo-2.10.0-21.el7_5.4")) flag++;
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"qemu-kvm-tools-rhev-2.10.0-21.el7_5.4")) flag++;
