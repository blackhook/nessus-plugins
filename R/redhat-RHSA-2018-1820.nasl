...
if (description)
{
  ...
  script_name(english:"RHEL 7 : Virtualization (RHSA-2018:1820)");
...
imgbased attempts to re-execute RPM %post scripts which involve
SELinux commands inside a namespace, and some commands failed due to
SELinux namespacing rules.

This update ensures that SELinux contexts inside imgbased update
namespaces now update appropriately, and the scripts are re-executed
by remounting /sys and / sys/fs/selinux inside the update namespace.
(BZ#1571607)

* Previously, vmcore files that were created by kdump were not
relabelled by kdumpctl after a reboot. As a result, Rsync returned an
error when trying to remove the security.selinux attribute while
syncing unlabelled files, as this is forbidden by selinux.

In this release, restorecon is run on the source directory before
syncing it, which enables the files to be synchronized with the
correct labels. (BZ# 1579141)

Enhancement(s) :

* Previously, imgbased filtered out stderr from LVM commands to
improve parsing reliability. In this release, imgbased now logs stderr
from LVM commands in order to provide this information in the imgbased
logs, for example during an upgrade failure. (BZ#1574187)"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2018:1820"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/security/cve/cve-2018-1124"
  );
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/security/cve/cve-2018-1126"
  );
...
else
{
  ...
  if (! (rpm_exists(release:"RHEL7", rpm:"redhat-virtualization-host-4.2"))) audit(AUDIT_PACKAGE_NOT_INSTALLED, "Virtualization");

  if (rpm_check(release:"RHEL7", reference:"imgbased-1.0.17-0.1.el7")) flag++;
  if (rpm_check(release:"RHEL7", reference:"python-imgbased-1.0.17-0.1.el7")) flag++;
  if (rpm_check(release:"RHEL7", cpu:"x86_64", reference:"redhat-release-virtualization-host-4.2-3.1.el7")) flag++;
  if (rpm_check(release:"RHEL7", reference:"redhat-virtualization-host-image-update-4.2-20180531.0.el7_5")) flag++;
  if (rpm_check(release:"RHEL7", reference:"redhat-virtualization-host-image-update-placeholder-4.2-3.1.el7")) flag++;
