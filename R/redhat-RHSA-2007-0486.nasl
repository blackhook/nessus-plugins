...
# extracted from Red Hat Security Advisory RHSA-2007:0486. The text 
...
if (description)
{
  ...
  script_xref(name:"RHSA", value:"2007:0486");

  script_name(english:"RHEL 2.1 : mod_perl (RHSA-2007:0486)");
...
before being used in a regular expression. If a server is configured
to use Apache::PerlRun, an attacker could request a carefully crafted
URI causing resource consumption, which could lead to a denial of
...
contain a backported fix to correct this issue."
  ...
  script_set_attribute(
    attribute:"see_also",
    value:"https://access.redhat.com/errata/RHSA-2007:0486"
  );
  script_set_attribute(
    attribute:"solution", 
...
{
  flag = 0;
  if (rpm_check(release:"RHEL2.1", cpu:"i386", reference:"mod_perl-1.26-8.el2")) flag++;
