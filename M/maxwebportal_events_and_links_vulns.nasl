...
"Unknown at this time." );
 ...
 script_set_attribute(attribute:"cvss_score_source", value:"CVE-2005-1017");
...
script_end_attributes();
  ...
  script_dependencies("cross_site_scripting.nasl", "http_version.nasl", "smtp_settings.nasl");
...
new_url = "http://www." + domain + "/" + gettimeofday();
...
foreach dir (cgi_dirs()) {
  ...
  w = http_send_recv3(method:"GET", item:dir + "/links_add_form.asp", port:port, exit_on_fail: 1);
  res = w[2];
  ...
  #     or accept them only from logged-in users.
    ...
    foreach match (split(matches)) {
      match = chomp(match);
      cat = pregmatch(pattern:pat, string:match, icase:TRUE);
      if (!isnull(cat)) {
        cat = cat[1];
        break;
      }
