...
the remote host.");
  script_set_attribute(attribute:"see_also", value:"https://www.4n6k.com/2014/02/forensics-quickie-pinpointing-recent.html");
...
foreach user (keys(reg_recentfile))
{
  foreach filetype (keys(reg_recentfile[user]))
  {
    foreach files (reg_recentfile[user][filetype])
    {
      ...
      if (REPORT_TO_UI)
      {
        report_extra_output = files['ascii'] + '\n';
      }
    }
  }
}
...
{
  sanitized_appfile = data_protection::sanitize_user_paths(report_text:appfile);
  appdata_rf_report += sanitized_appfile + '\n';
  if (REPORT_TO_UI)
  {
    report_extra_output = sanitized_appfile + '\n';
  }
}
...
reg_rf_report = 'user,filetype,raw,ascii,hex\n' +
                data_protection::sanitize_user_paths(report_text:reg_rf_report);
