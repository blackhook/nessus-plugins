...
# extracted from Mozilla Foundation Security Advisory mfsa2022-49.
...
if (description)
{
  ...
  script_name(english:"Mozilla Thunderbird < 102.5");

  script_set_attribute(attribute:"synopsis", value:
"A mail client installed on the remote macOS or Mac OS X host is affected by multiple vulnerabilities.");
  script_set_attribute(attribute:"description", value:
"The version of Thunderbird installed on the remote macOS or Mac OS X host is prior to 102.5. It is, therefore, affected
by multiple vulnerabilities as referenced in the mfsa2022-49 advisory.
  ...
  - Cross-Site Tracing occurs when a server will echo a request back via the Trace method, allowing an XSS
    ...
    applied the same mitigations to the use of this and similar headers. (CVE-2022-45411)
  ...
  - Mozilla developers Andrew McCreight and Gabriele Svelto reported memory safety bugs present in Thunderbird
    ...
    of these could have been exploited to run arbitrary code. (CVE-2022-45421)

Note that Nessus has not tested for these issues but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://www.mozilla.org/en-US/security/advisories/mfsa2022-49/");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Mozilla Thunderbird version 102.5 or later.");
...
mozilla_check_version(version:version, path:path, product:'thunderbird', esr:FALSE, fix:'102.5', severity:SECURITY_HOLE);
