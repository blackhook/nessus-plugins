...
if (description)
{
  ...
  script_name(english:"ManageEngine ServiceDesk Plus Default Administrator Credentials");
...
credentials."
  ...
  script_set_attribute(
    ...
    value:
"The remote ManageEngine ServiceDesk Plus install uses a default set
of credentials ('administrator' / 'administrator') to control access
...
if ("form action='j_security_check" >!< res[2])
  exit(1, "The ManageEngine ServiceDesk Plus install at "+install_url+" has an unexpected form.");

# Make sure we have a session cookie.
val = get_http_cookie(name:"JSESSIONID");
if (isnull(val)) exit(1, "Failed to extract the session cookie from the ManageEngine ServiceDesk Plus install at "+install_url+".");
...
if (
  ...
  egrep(pattern:">Log out<.+"+user, string:res2[2])
)
{
...
else audit(AUDIT_WEB_APP_NOT_AFFECTED, "ManageEngine ServiceDesk Plus", install_url);
