...
vulnerabilities :
  ...
  script_set_attribute(attribute:"see_also", value:"https://www.mcafee.com/enterprise/en-us/products/dlp-endpoint.html");
...
match = pregmatch(string:data, pattern:'docBase="([^"]+)"');
if (!isnull(match))
{
...
{
  match = pregmatch(string:data, pattern:"extension\.version\s*=\s*([\d.]+)");
