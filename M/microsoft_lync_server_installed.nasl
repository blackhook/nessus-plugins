...
if (description)
{
  ...
  script_xref(name:"IAVT", value:"0001-T-0662");
...
if (!empty_or_null(list))
{
  foreach key (keys(list))
  {
    if ('Microsoft Office Live Meeting 2007' >< list[key])
    {
      ...
      {
        key = str_replace(string:key, find:"SOFTWARE\Microsoft", replace:"SOFTWARE\Wow6432Node\Microsoft");
        path = get_registry_value(handle:hklm, item:key);
      }
      if (!empty_or_null(path))
        installs["live_meeting_2007"] = path;
    }
    else if ('Microsoft Conferencing Add-in' >< list[key])
    {
      key = "Microsoft\Live Meeting\Addins";
      subkeys = get_reg_name_value_table(handle:hklm, key:"SOFTWARE\"+key);
      foreach sub (keys(subkeys))
      {
          if(sub =~ "InstallDir")
            path = subkeys[sub];
      }
      if(empty_or_null(path) && arch == "x64")
      {
        subkeys = get_reg_name_value_table(handle:hklm, key:"SOFTWARE\Wow6432Node\"+key);
        foreach sub (keys(subkeys))
        {
          if(sub =~ "InstallDir")
            path = subkeys[sub];
      ...
      if (!empty_or_null(path))
        installs["live_meeting_2007_addin"] = path;
    }
  }
}
...
foreach ver (keys(installs["server"]))
{
  ...
  if (!empty_or_null(fver['value']))
  {
    ...
    if(mver == "6.0")
      extra["Product"] = "Skype for Business Server 2015";
...
if (!empty_or_null(installs["live_meeting_2007_addin"]))
{
  path = installs["live_meeting_2007_addin"];
  dll = hotfix_append_path(path:path, value:"LMAddins.dll");
  fver = hotfix_get_fversion(path:dll);
  err = hotfix_handle_error(error_code:fver['error'], file:dll, appname:appname, exit_on_fail:FALSE);
  ...
  if (!empty_or_null(fver['value']))
  {
    register_install(
      ...
      path:path,
      version:join(fver["value"], sep:"."),
      extra:make_array("Product", "Live Meeting 2007 Add-in"),
      cpe:"cpe:/a:microsoft:live_meeting" 
