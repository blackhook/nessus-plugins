...
flavour = get_kb_item_or_exit("MacOSX/Sophos/Product");
...
if (flavour == 'Endpoint')
{
  info = get_av_info("macosx_sophos_managed");
}
else if (flavour == 'Home')
{
  info = get_av_info("macosx_sophos_home");
}
else
{
...
match = pregmatch(pattern:"^([0-9]+\.[0-9]+\.[0-9]+).*$", string:version);
...
if (isnull(match) || isnull(info[match[1]]["latest_prod_ver"]))
{
  ...
  match = pregmatch(pattern:"^([0-9]+\.[0-9]+).*$", string:version);
  if (isnull(match) || isnull(info[match[1]]["latest_prod_ver"])) audit(AUDIT_UNKNOWN_APP_VER, "Sophos Anti-Virus");
}
prod = match[1];
...
if (hole)
{
  if (report_verbosity > 0) security_report_v4(port:0, extra:report, severity:SECURITY_HOLE);
  else security_report_v4(port:0, severity:SECURITY_HOLE);
}
else
{
