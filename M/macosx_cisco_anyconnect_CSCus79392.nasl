...
if (description)
{
  ...
  script_name(english:"Mac OS X : Cisco AnyConnect Secure Mobility Client < 3.1.10010.0 / 4.0.x < 4.0.4013.0 / 4.1.x < 4.1.4011.0 IPC File Write Vulnerability");
...
prior to 4.0.4013.0, or version 4.1.x prior to 4.1.4011.0. It is,
therefore, affected by a flaw that allows unauthenticated IPC commands
to write files. A local, authenticated attacker, using a specially
crafted IPC command, can write or overwrite arbitrary files.");
