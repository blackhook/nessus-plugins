...
if (description)
{
  ...
  script_bugtraq_id(102112);
  script_xref(name:"MFSA", value:"2017-28");

  script_name(english:"Mozilla Firefox ESR < 52.5.2 Private Mode Fingerprinting Vulnerability (macOS)");
...
Mac OS X host is prior to 52.5.2. It is, therefore, affected by a
flaw related to handling Private Mode, web workers, and IndexedDB
access that allows an attacker to cause persistent data to exist that
can uniquely fingerprint a user.");
...
mozilla_check_version(version:version, path:path, product:'firefox', esr:TRUE, fix:'52.5.2', min:'52', severity:SECURITY_WARNING);
