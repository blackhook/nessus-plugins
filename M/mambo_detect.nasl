...
in PHP.");
  script_set_attribute(attribute:"see_also", value:"http://mambo-foundation.org/");
...
foreach dir (dirs)
{
  ...
  r = http_send_recv3(method: "GET", item:dir + "/administrator/index.php", port:port);
  if (isnull(r)) exit(0);
    ...
    if (r[0] =~ "^HTTP/.* 200 OK")
    {
	  ...
	  get_http_cookie(name: "mosvisitor") || # == "1"
        ...
        )
        {
          ...
          r = http_send_recv3(method: "GET", item:dir + "/includes/js/mambojavascript.js", port:port);
          if (isnull(r)) exit(0);
          ...
          else
          {
            ...
            r = http_send_recv3(method: "GET", item:dir + "/images/stories/mambositeserver.gif", port:port);
            if (isnull(r)) exit(0);
	    res2 = r[2];
