...
if (description)
{
  ...
  script_bugtraq_id(96014, 97940, 98050);
  script_xref(name:"MFSA", value:"2017-13");

  script_name(english:"Mozilla Thunderbird < 52.1 Multiple Vulnerabilities (macOS)");
...
Mac OS X host is prior to 52.1. It is, therefore, affected by multiple
vulnerabilities :
...
if (!isnull(is_esr)) exit(0, 'The Mozilla Thunderbird installation is in the ESR branch.');

mozilla_check_version(version:version, path:path, product:'thunderbird', fix:'52.1', severity:SECURITY_HOLE, xss:true);
