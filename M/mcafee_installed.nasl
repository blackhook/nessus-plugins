...
if (product_path)
{
  dat_update = check_item(reg:"SOFTWARE\McAfee\DesktopProtection\Tasks\{A14CD6FC-3BA8-4703-87BF-E3247CE382F5}", item:"bUpdateDAT");
...
#
report = "The remote host has the McAfee antivirus installed.
...
{
  date_items = pregmatch(string:database_date, pattern:"^(\d\d)-(\D\D\D)-(\d\d\d\d)$");
  if (!empty_or_null(date_items))
  {
    ...
    month_name = _months_abrev[month];
    month_num = month_num_by_name(month_name, base:1);
