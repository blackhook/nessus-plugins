...
if (description)
{
  ...
  script_xref(name:"IAVT", value:"2009-T-0010-S");
...
"Upgrade the firmware according to the vendor's advisory.");
  ...
  script_require_keys("www/hp_laserjet/pname", "www/hp_laserjet/fw", "www/hp_laserjet/port");
...
printer_port = get_kb_item("www/hp_laserjet/port");
if (isnull(printer_port)) exit(1, "The 'www/hp_laserjet/port' KB item is missing.");
...
)
{
  if (report_verbosity > 0)
  {
    ...
    security_hole(port:printer_port, extra:info);
  }
  else security_hole(port:printer_port);
}
