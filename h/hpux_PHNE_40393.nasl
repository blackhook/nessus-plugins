...
sendmail_fingerprint = get_kb_item("Host/OS/SMTP/Fingerprint");
if (empty_or_null(sendmail_fingerprint)) audit(AUDIT_UNKNOWN_APP_VER, "sendmail");
sendmail_pattern = "Sendmail\s*version\s*8\.9\.3(\D|$)";
sendmail_match = pregmatch(pattern:sendmail_pattern, string:sendmail_fingerprint, icase:TRUE);
if (empty_or_null(sendmail_match)) audit(AUDIT_OS_NOT, "running sendmail 8.9.3");
