...
# extracted from HP patch PHNE_30168. The text itself is
...
if (description)
{
  ...
  script_name(english:"HP-UX PHNE_30168 : HP-UX Running RPC, Remote Unauthorized Access or Denial of Service (DoS) (HPSBUX00252 SSRT2439 rev.13)");
...
Potential buffer overflow in xdrmem_getbytes() and related functions."
  ...
  script_set_attribute(
    attribute:"solution", 
    value:"Install patch PHNE_30168 or subsequent."
  );
...
{
  exit(0, "The host is not affected since PHNE_30168 applies to a different OS release.");
}

patches = make_list("PHNE_30168", "PHNE_30808");
...
if (hpux_check_patch(app:"NFS.NFS-CLIENT", version:"B.11.04")) flag++;
