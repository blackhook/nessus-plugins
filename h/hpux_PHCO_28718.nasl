...
# extracted from HP patch PHCO_28718. The text itself is
...
if (description)
{
  ...
  script_name(english:"HP-UX PHCO_28718 : HP-UX Running wall(1), Local Privilege Increase, Denial of Service (DoS) (HPSBUX00258 SSRT3483 rev.2)");
...
wall(1M) contains a locally exploitable vulnerability."
  ...
  script_set_attribute(
    attribute:"solution", 
    value:"Install patch PHCO_28718 or subsequent."
  );
...
{
  exit(0, "The host is not affected since PHCO_28718 applies to a different OS release.");
}

patches = make_list("PHCO_28718");
