...
web-hosting automation suite for the Windows Server family platform.");
 script_set_attribute(attribute:"see_also", value:"https://hostingcontroller.com/");
...
script_set_attribute(attribute:"cpe", value:"cpe:/a:hosting_controller:hosting_controller");
...
if (!isnull(ver) && !isnull(hc_port))
{
  if (hotfix) ver = ver + " hotfix " + hotfix;

  set_kb_item(
    name:"www/" + hc_port + "/hosting_controller",
    value:ver
  );
  set_kb_item(name:"Services/hosting_controller", value:hc_port);

  report = 'Hosting Controller ' + ver + ' was detected on the remote host with its\n' +
