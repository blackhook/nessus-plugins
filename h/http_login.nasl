...
script_add_preference(name:"Abort web application tests if login fails", type:"checkbox", value:"no");
 script_dependencies("find_service1.nasl", "httpver.nasl", "logins.nasl", "web_app_test_settings.nasl", "broken_web_server.nasl");
...
cookies = rq["Cookie"];
if (cookies)
  set_kb_item(name: "/tmp/http/auth/" + port, value: 'Cookie: '+cookies);
else
{
