...
# extracted from HP patch PHSS_33253. The text itself is
...
if (description)
{
  ...
  script_name(english:"HP-UX PHSS_33253 : HP OpenView Operations, Remote Unauthorized Access and Denial of Service (DoS) (HPSBMA02149 SSRT050968 rev.1)");
...
to allow unauthorized access or to create a Denial of Service (DoS)."
  ...
  script_set_attribute(
    attribute:"solution", 
    value:"Install patch PHSS_33253 or subsequent."
  );
...
{
  exit(0, "The host is not affected since PHSS_33253 applies to a different OS release.");
}

patches = make_list("PHSS_33253");
...
flag = 0;
if (hpux_check_patch(app:"OVOPC-CLT.OVOPC-UXIA-CLT", version:"A.07.10")) flag++;
