...
if (description)
{
  ...
  script_name(english:"ESXi 7.0 DoS (VMSA-2020-0029)");

  script_set_attribute(attribute:"synopsis", value:
"The remote VMware ESXi host is missing a security patch and is affected by a DoS vulnerability");
  script_set_attribute(attribute:"description", value:
"According to its self-reported version number, the remote VMware ESXi host is version 7.0. It is, therefore, affected
by a DoS vulnerability due to improper input validation in GuestInfo. A malicious actor with normal user privilege
access to a virtual machine can crash the virtual machine's vmx process leading to a denial of service condition.
...
if (isnull(match)) audit(AUDIT_UNKNOWN_BUILD, 'VMware ESXi', '7.0');
ver = match[1];

if (ver !~ "^(7\.0)$") audit(AUDIT_OS_NOT, 'ESXi 7.0');
