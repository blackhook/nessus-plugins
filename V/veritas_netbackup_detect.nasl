...
"n/a");
  ...
  script_set_attribute(attribute:"cpe", value:"cpe:/a:symantec:veritas_netbackup");
...
function check (port, socket, line)
{
 ...
 if (egrep(pattern:"^NetBackup .* - bpjava-.*svc.*", string:line))
 {
  ...
  register_install(
    app_name: "Veritas Net Backup Java Agent",
    vendor : 'Symantec',
    product : 'Veritas Netbackup',
    version: version,
    port: port,
    protocol: "tcp",
    service: "VeritasNetBackup",
    cpe:"cpe:/a:symantec:veritas_netbackup");
