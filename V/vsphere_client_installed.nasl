...
vSphere Server, is installed on the remote Windows host.");
  script_set_attribute(attribute:"see_also", value:"https://www.vmware.com/support/vsphere.html");
  ...
  if (!isnull(fsize))
  {
    while (off <= fsize)
    {
      ...
      if (pat)
      {
        ...
        matches = pregmatch(pattern:pat, string:data);
        if (!isnull(matches))
        {
          version = matches[1];
          build = matches[2];
