...
function check_kins(pdir)
{
  ...
  while (!isnull(iter[1]))
  {
    ...
    if (pregmatch(pattern:"^[A-Z][a-z]{3,5}$", string:dir))
    {
      tmp_dir = FALSE;
      exe_dir = FALSE;
      dll_dir = FALSE;
      ...
      while (!isnull(iter2[1]))
      {
        ...
        if (!exe_dir && pregmatch(pattern:"^[a-z]{4,5}\.exe$", string:file))
        {
          exe_dir = TRUE;
        ...
        # lowercase letters 4 to 5 letters long.
        else if (!tmp_dir && pregmatch(pattern:"^[a-z]{4,5}\.tmp$", string:file))
        {
          tmp_dir = TRUE;
          kins_files[max_index(kins_files)] = file;
        }
        ...
        else if(pregmatch(pattern:"^.{3}\.dll$", string:file))
        {
          dll_dir = TRUE;
          kins_files[max_index(kins_files)] = file;
        }
