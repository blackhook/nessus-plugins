...
if (description)
{
  ...
  script_xref(name:"IAVT", value:"0001-T-0576");
...
global_var instances, name, port, domain, files_to_find;
...
function find_files_in_jar()
{
  ...
  while (i < jar_len)
  {
    ...
    if (magic != 0x04034b50)
    {
      spad_log(message:"Bad magic: " + hexstr(magic));
      return NULL;
    }
    ...
    if (files_to_find[filename])
    {
      ...
      else
      {
        ...
        spad_log(message:"Unknown compression method: " + compression);
        break;
      }
    }
  ...
  {
    if (files_found == 0)
      spad_log(message:"The desired files weren't found in the jar file.");
    else
      spad_log(message:"Only " + join(files, sep:'') + " was found in the jar file.");
...
function check_instance()
{
  ...
  if (rc != 1)
    spad_log(message:'get_instances: Can\'t connect to '+share+' share.');
  else
  {
    get_version(name);
...
function get_version()
{
  ...
  if (fh)
  {
    ...
    if (!isnull(files))
    {
      match = pregmatch(string:files['META-INF/MANIFEST.MF'], pattern:'Implementation-Version: ([0-9]+)');
      if (!isnull(match))
      {
        build = match[1];
      ...
      match = pregmatch(string:file_data, pattern:'([0-9]+,[0-9]+,[0-9]+),');
      if (!isnull(match))
      {
  ...
  else spad_log(message:'Unable to open: ' + path);
}
...
function get_hotfixes()
{
  ...
  while (!isnull(fh[1]))
  {
    ...
    if (fh[2] & FILE_ATTRIBUTE_DIRECTORY == 0)
    {
      ...
      match = pregmatch(string:fh[1], pattern:"^(c?hf)" + ver + "-?([0-9]+).jar$");
      if (!isnull(match))
      {
        type = match[1];
        id = match[2];
...
function get_instances()
{
  ...
  if (!isnull(fh))
  {
    ...
    foreach line (split(blob, sep:'\n', keep:FALSE))
    {
      ...
      match = pregmatch(string:line, pattern:'<name>([^<]+)</name>');
      if (!isnull(match))
      {
        name = match[1];
        continue;
      ...
      match = pregmatch(string:line, pattern:'<directory>([^<]+)</directory>');
      if (!isnull(match))
      {
  ...
  else spad_log(message:'Unable to open ' + dir + '\\lib\\servers.xml');
...
function get_connector_path()
{
  ...
  while (!isnull(bat[1]))
  {
    ...
    if (!isnull(fh))
    {
      ...
      match = pregmatch(string:blob, pattern:'-cfwebroot *"([^"]+)"');
      if (!isnull(match))
      {
        webroot = match[1];
        break;
...
foreach key (keys(list))
{
  ...
  # version < 8 does not have this key it seems
  if (prod =~ 'Adobe ColdFusion [89] with JRun 4' ||
      prod =~ 'Adobe ColdFusion 2018' ||
      prod =~ 'Adobe ColdFusion 2016')
  {
  ...
  if (!isnull(key_h))
  {
    ...
    if(isnull(item))
    {
      ...
      if(!isnull(item))
      {
        ...
        match = pregmatch(pattern:"^(.+)\\[^\\]*$", string:tmp);
...
keys = make_list('SOFTWARE\\Adobe\\Install Data', 'SOFTWARE\\Macromedia\\Install Data', 'SOFTWARE\\Wow6432Node\\Adobe\\Install Data');
...
foreach key (keys)
{
  ...
  if (!isnull(key_h))
  {
    ...
    for (i=0; i<info[1]; ++i)
    {
      ...
      spad_log(message:"Subkey found: " +subkey);
      match = pregmatch(string:subkey, pattern:"^(Adobe ColdFusion|ColdFusion MX) (\d+)$");
...
foreach name (keys(instances))
{
  ...
  spad_log(message:"Checking instance: "+name);
  ver = instances[name]['version'];
