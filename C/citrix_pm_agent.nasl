...
if (description)
{
 ...
 script_cvs_date("Date: 2018/11/15 20:50:26");
  ...
  script_name(english:"Citrix Password Manager Agent Secondary Credentials Information Disclosure");
  script_summary(english:"Checks version of Citrix Password Manager Agent");

  script_set_attribute(attribute:"synopsis", value:
"The remote Windows host has an application that is affected by an
information disclosure vulnerability.");
  script_set_attribute(attribute:"description", value:
"Citrix Password Manager Agent is installed on the remote host.

The version of Citrix Password Manager Agent on the remote host is
reportedly affected by an information disclosure vulnerability
involving secondary credentials.");
  script_set_attribute(attribute:"see_also", value:"https://support.citrix.com/article/CTX120743");
...
key = "SOFTWARE\Citrix\MetaFrame Password Manager\Shell";
...
if (!isnull(fh))
{
  ...
  # There's a problem if the version of Citrix Password Manager Agent is earlier than 4.6.264
  if (!isnull(ver))
  {
    if (ver[0] < 4 || ( ver[0] == 4 && ver[1] < 6) || ( ver[0] == 4 && ver[1] == 6 && ver[2] < 264 ))
    {
      ...
      report = string(
        "\n",
        "Version ", version, " of the Citrix Password Manager Agent is installed under :\n",
        "\n",
        "  ", path, "\n"
      );
