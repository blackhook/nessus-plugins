...
more of the following vulnerabilities :
  ...
  # https://tools.cisco.com/security/center/content/CiscoSecurityAdvisory/cisco-sa-20140305-wlc
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?788c0675");
  script_set_attribute(
    attribute:"solution",
    value:
...
upgrade recommendations."
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2014-0701");
...
sysinfo = get_kb_item_or_exit("Host/Cisco/show sysinfo");
...
if (
  !(
    ...
    (model =~ "(^|[^0-9])75\d\d($|[^0-9])" && device_model !~ "^c") ||
    model =~ "(^|[^0-9])85\d\d($|[^0-9])" ||
    "AIR-WLC" >< model ||
    (model =~ "(^|[^0-9])(65\d\d|76\d\d|3750G)($|[^0-9])" && ("catalyst" >< tolower(sysinfo)))
  )
