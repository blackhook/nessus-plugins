...
if (description)
{
  ...
  script_cve_id("CVE-2014-2142");
  script_bugtraq_id(66686);
  script_xref(name:"CISCO-BUG-ID", value:"CSCun06870");
  script_xref(name:"IAVB", value:"2014-B-0038");

  script_name(english:"Cisco ONS 15454 Controller Card DoS (CSCun06870)");
...
reset, resulting in a denial of service condition.");
  # https://tools.cisco.com/security/center/viewAlert.x?alertId=33679
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?e1759a64");
  script_set_attribute(attribute:"see_also",value:"https://tools.cisco.com/bugsearch/bug/CSCun06870");
...
else if (version_formatted =~ "^9\.[01346]\.0(\.|$)") flag++;
else if (version_formatted =~ "^9\.2\.[0-2](\.|$)") flag++;
else if (version_formatted =~ "^9\.8\.[01](\.|$)") flag++;
else if (version_formatted =~ "^10\.0\.0(\.|$)") flag++;
