...
if (description)
{
  ...
  script_cve_id("CVE-2013-3377");
  script_bugtraq_id(60678);

  script_name(english:"Cisco TelePresence CVE-2013-3377 Malformed SIP Packet Handling Remote DoS");
  ...
  script_set_attribute(
    ...
    value:
"According the self-reported version returned by the SNMP service on the
remote device, it is affected by a denial of service vulnerability that
can be triggered by sending a specially crafted SIP packet."
  );
...
)
{
  ...
  if (
    !isnull(item) &&
    ver_compare(ver:item[1], fix:"5.1.7", strict:FALSE) == -1
  )
  {
    version = item[0];
    fix = "TC5.1.7";
