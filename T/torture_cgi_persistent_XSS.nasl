...
information to the application.  Nessus cannot test them all.");
  script_set_attribute(attribute:"see_also", value:"https://en.wikipedia.org/wiki/Cross_site_scripting#Persistent");
...
global_var flaw_cnt, seen_req, visible_on;
...
function test_local(port, url, cgi, meth, postdata, poisoned_param)
{
  ...
  foreach url (act)
  {
    ...
    if (r[0] =~ "^HTTP/1\.[01] 400 ")
      debug_print(level:2, 'test_local(', url, ') = ', r[0]);
...
function test1url_local(port, url, poisoned_param)
{
  ...
  if (try_all_meth || meth_h["get"])
    {
  e = test_local(port:port, meth: "GET", url: u, cgi: cgi, poisoned_param: poisoned_param);
  if (e >= 0) return e;
  }
  ...
  if (try_all_meth || meth_h["post"])
    ...
    u = str_replace(string: url, find: "$URL$", replace: "P"+url16);
      e = test_local(port: port, meth: 'POST', url: cgi, cgi: cgi, postdata:mypostdata, poisoned_param: poisoned_param);
      return e;
    }
  return -1;
}

function test_cgi_rec_local(port, url, param_l, data_ll, idx, poison_idx, var_idx)
{
  local_var	i, j, d, u, e, fl, val, head, cnt;

  #display("test_cgi_rec_local: port=", port, " url=", url, " idx=", idx, " poison_idx=", poison_idx, " var_idx=", var_idx,"\n");

  if (isnull(param_l[idx]))	# last argument
    return test1url_local(port: port, url: url, poisoned_param: param_l[poison_idx]);
  ...
  if (idx == poison_idx)
  {
    foreach fl (poison)
     for (i = 0; ! isnull(d[i]); i ++)
     {
       ...
       e = test_cgi_rec_local(port:port, url: u, param_l: param_l, data_ll: data_ll, idx: idx + 1, poison_idx: poison_idx, var_idx: var_idx);
       cnt ++;
       if (e >= 0)
...
function test1cgi_local(port, cgi, param_l, data_ll)
{
  local_var	i, e, cnt;
  ...
  for (i = 0; ! isnull(param_l[i]); i ++)
  {
    ...
    {
      debug_print(level:2, "test1cgi_local port=",port, " cgi=", cgi, " vul=XP -- flaw has already been reported");
      return -1;
    }
    ...
    init_cookiejar();
    e = test_cgi_rec_local(port:port, url: strcat(cgi, "?"), param_l: param_l, data_ll: data_ll, idx: 0, poison_idx: i, var_idx: -1);
    cnt ++;
    if (! e) return 0;
...
global_var visible_on;

function torture_cgis_local(port)
{
  ...
  {
    debug_print("torture_cgis_local: web app tests are disabled on port ", port);
    return NULL;
  }
  ...
  foreach cgi_name (cgis)
  {
   ...
   e = test1cgi_local(port:port, cgi: cgi_name, param_l: args_l, data_ll: vals_l);
   if (! e)
    if (++ cnx_errors > 32)
     break;
    else
  ...
  if (url_count > 0 && (flaw_cnt == 0 || thorough_tests))
  {
    ...
    foreach u (url_l)
      ...
      {
        prev_url = u;
        e = test1url_local(port: port, url: u);
...
set_torture_cgis(func:@torture_cgis_local);

rep = torture_cgis_local(port: port);
if (rep)
{
